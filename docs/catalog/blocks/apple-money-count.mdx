...
```text
📷Frames by HeyGen Make me a five-second video of, on a white background, of a Apple-style bold font counting from $0 to $10,000. Once it counts to $10,000, it changes to a green color and the screen also flashes green for a second, and then money icons come out of the $10,000 amount all over the screen and then disappear.
```

<video className="w-full aspect-video rounded-xl object-cover bg-zinc-100 dark:bg-zinc-800" src="https://static.heygen.ai/frames-oss/docs/images/catalog/blocks/apple-money-count.mp4" poster="https://static.heygen.ai/frames-oss/docs/images/catalog/blocks/apple-money-count.png" autoPlay muted loop playsInline />
...
```bash Terminal
npx frames add apple-money-count
```
...
| File | Target | Type |
| --- | --- | --- |
| `apple-money-count.html` | `compositions/apple-money-count.html` | frames:composition |
| `assets/sfx-production.wav` | `assets/sfx-production.wav` | frames:asset |
