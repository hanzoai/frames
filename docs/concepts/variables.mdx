...
<script>
  document.getElementById("hero").src = __frames.getVariables().heroVideo;
</script>
```
...
<video id="hero" data-start="0" data-track-index="0"></video>
<script>
  const { heroVideo, heroDuration } = __frames.getVariables();
...
| Frame rate | `--fps` flag on `hanzo frame render`, or `config.fps` in the SDK |
...
Inside any composition script, call `window.__frames.getVariables()` to get the resolved variable values. The return type is `Partial<Record<string, unknown>>` — use destructuring with defaults matching the declared `default` values:
...
<html data-composition-variables='[
  ...
  <body>
    <div data-composition-id="card" data-width="1920" data-height="1080">
      ...
      <script>
        const { title = "Untitled", color = "#111827" } = __frames.getVariables();
...
`__frames.getVariables()` is a shorthand for `window.__frames.getVariables()` and works in both top-level and sub-composition scripts. The runtime automatically scopes sub-compositions so each instance sees its own resolved values.
...
```bash Terminal
# Inline JSON
npx @hanzo/frame render --variables '{"title":"Q4 Report","color":"#1d4ed8"}' --output q4.mp4

# JSON file
npx @hanzo/frame render --variables-file ./vars.json --output out.mp4

# Fail on undeclared or mistyped variables
npx @hanzo/frame render --variables '{"title":"Q4 Report"}' --strict-variables --output out.mp4
```
...
```bash Terminal
npx @hanzo/frame render --batch rows.json --output "renders/{name}.mp4" --strict-variables
```
...
| CLI `--variables` flag | Highest | `hanzo frame render --variables '{...}'` |
...
If you are building tooling on top of `@frames/core`, the variable declarations are readable without rendering:
...
- **Handoff** — copy the effective values as JSON or as a ready-to-run
  `hanzo frame render --variables` command.
