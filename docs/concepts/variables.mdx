...
| Frame rate | `--fps` flag on `frames render`, or `config.fps` in the SDK |
...
```bash Terminal
# Inline JSON
npx @hanzo/frames render --variables '{"title":"Q4 Report","color":"#1d4ed8"}' --output q4.mp4

# JSON file
npx @hanzo/frames render --variables-file ./vars.json --output out.mp4

# Fail on undeclared or mistyped variables
npx @hanzo/frames render --variables '{"title":"Q4 Report"}' --strict-variables --output out.mp4
```
...
```bash Terminal
npx @hanzo/frames render --batch rows.json --output "renders/{name}.mp4" --strict-variables
```
...
| CLI `--variables` flag | Highest | `frames render --variables '{...}'` |
...
- **Handoff** — copy the effective values as JSON or as a ready-to-run
  `frames render --variables` command.
