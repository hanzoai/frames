...
```bash Terminal
# Inline JSON
npx frames render --variables '{"title":"Q4 Report","color":"#1d4ed8"}' --output q4.mp4

# JSON file
npx frames render --variables-file ./vars.json --output out.mp4

# Fail on undeclared or mistyped variables
npx frames render --variables '{"title":"Q4 Report"}' --strict-variables --output out.mp4
```
...
```bash Terminal
npx frames render --batch rows.json --output "renders/{name}.mp4" --strict-variables
```
