...
<Info>
**Quick version** — Fork the repo. Write one HTML file with a paused GSAP timeline. Add `registry-item.json`. Run `frames lint` + `validate`. Publish with `npx frames publish`. Open a PR.
</Info>
...
```bash
npx frames add instagram-follow
```
