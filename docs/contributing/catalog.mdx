...
description: How to add blocks and components to the Frames registry.
---

Your agent already knows how to build video components. It writes HTML. Frames renders it. The registry is the collection of everything that's been built — 52 blocks and counting.

This guide shows you how to add to it.

<Info>
**Quick version** — Fork the repo. Write one HTML file with a paused GSAP timeline. Add `registry-item.json`. Run `hanzo frame lint` + `validate`. Publish with `npx @hanzo/frame publish`. Open a PR.
</Info>

## Why Contribute?

Every block in the registry exists because someone needed it and built it. When you add a block, every Frames user gets it with one command:

```bash
npx @hanzo/frame add instagram-follow
```

The registry grows, Frames gets more useful, and your work ships to everyone.
...
Open an issue on [GitHub](https://github.com/hanzoai/frames/issues) with a visual reference. Tag it `component-request`.
...
If you use Claude Code with Frames skills:

> "I want to contribute a new transition that looks like \[description\]"

The `/frames-registry` skill scaffolds the structure, validates, renders a preview, publishes to [frames.hanzo.ai](https://frames.hanzo.ai), and prepares the PR.
...
<Steps>
  <Step title="Fork and create">
    Fork [hanzoai/frames](https://github.com/hanzoai/frames) and create your block directory:
    ```bash
  ...
  <Step title="Publish and PR">
    ...
    ```
    Open a PR with your [frames.hanzo.ai](https://frames.hanzo.ai) preview link.
  </Step>
</Steps>

**External contributors:** attach the preview MP4 to your PR. A maintainer handles the catalog image.

Previews are served from the docs site's own `/images/` path, so a regenerate is all that is needed — nothing is uploaded anywhere.
