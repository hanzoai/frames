...
When you modify the CLI or any package it bundles (core, engine, producer, studio), you need to test those changes against real projects _outside_ the monorepo — the same way an end user would run `hanzo frame preview`.
...
npm uninstall -g frames 2>/dev/null
...
```bash
npm uninstall -g frames
cd packages/cli && bun link
```

**Port already in use**

`hanzo frame preview` defaults to port 3002 and auto-increments if it's taken. Pass `--port` to use a specific port:

```bash
hanzo frame preview . --port 4000
```
