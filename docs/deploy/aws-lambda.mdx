...
| AWS SAM CLI | `frames lambda deploy/destroy` shells out to `sam deploy`/`sam delete`. | [Install guide](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/install-sam-cli.html) |
...
### Path 1 — `frames lambda` CLI (recommended)
...
The `--wait` flag blocks and streams per-chunk progress + accrued cost; drop it to fire-and-forget, then poll with `frames lambda progress <renderId>` on your own cadence.
...
frames lambda render ./my-project --site-id=a1b2c3d4e5f6g7h8 \
...
Lambda renders are billed by GB-seconds (Lambda billed duration × configured memory) plus a tiny per-state-transition fee for Step Functions standard workflows. `frames lambda progress` exposes the running tally:

```bash
frames lambda progress my-render-id
# Status:    SUCCEEDED
# Progress:  100%
# Frames:    480 / 480
# Lambdas:   5
...
The IAM credential running `lambda deploy` doesn't have permission to create the service role CloudFormation needs. Run `frames lambda policies user` and attach the printed policy to your IAM user (or take the `policies role` output and have your admin create a deploy role).

### `Lambda function failed: PLAN_HASH_MISMATCH`

Step Functions invoked a `renderChunk` with a plan hash that didn't match the planDir on S3. Almost always means the producer version differs between the local `plan()` build and the deployed Lambda ZIP. Re-run `frames lambda deploy` (which rebuilds the ZIP) and re-render.
...
frames lambda deploy --stack-name=<your-stack>
```
...
The render bucket is created with CloudFormation `Retain` on delete — `frames lambda destroy` (or `sam delete`) tears the function + state machine down but the bucket survives. This is intentional: it protects final-rendered MP4s from being lost when you re-deploy. To fully reclaim storage, empty + delete the bucket via the AWS console / `aws s3 rb`.

## What's NOT in the v1 surface

- **Webhooks on completion.** Not in v1 — poll with `frames lambda progress` or watch the Step Functions execution. A `--webhook` flag with an SNS topic is on the Phase 6c backlog.
