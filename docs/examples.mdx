...
```bash Terminal
npx @hanzo/frames init my-video --example <name>
```
...
```bash Terminal
npx @hanzo/frames init my-video --example warm-grain --video ./my-clip.mp4
```
...
<CardGroup cols={2}>
  ...
  <Card title="Launch Videos" icon="github" href="/launch-videos">
    Real production launch projects — open-source for you to read and remix.
  </Card>
