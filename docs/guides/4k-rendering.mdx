...
<CardGroup cols={2}>
  <Card title="Author at 4K" icon="ruler">
    ...
    ```bash
    npx @hanzo/frames init my-video --resolution 4k
    ```
  </Card>
  <Card title="Supersample at render" icon="up-right-and-down-left-from-center">
    ...
    ```bash
    npx @hanzo/frames render --resolution 4k --output 4k.mp4
    ```
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Render an existing project at 4K">
    ```bash Terminal
    npx @hanzo/frames render --resolution 4k --output my-video-4k.mp4
    ```
...
```bash Terminal
npx @hanzo/frames render --resolution 4k         # landscape 4K
npx @hanzo/frames render --resolution portrait-4k # vertical 4K (TikTok / Reels at max quality)
npx @hanzo/frames render --resolution 1080p       # explicit 1080p (no-op on 1080p compositions)
```
...
```bash
# OK — both landscape
frames render --resolution 4k         # composition is 1920×1080

# Error — composition is landscape, target is portrait
frames render --resolution portrait-4k  # composition is 1920×1080
...
- **New project**: `frames init my-video --resolution 4k`
- **Existing project**: `frames render --resolution 4k --output 4k.mp4`
