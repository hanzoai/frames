...
<CardGroup cols={2}>
  <Card title="Author at 4K" icon="ruler">
    ...
    ```bash
    npx frames init my-video --resolution 4k
    ```
  </Card>
  <Card title="Supersample at render" icon="up-right-and-down-left-from-center">
    ...
    ```bash
    npx frames render --resolution 4k --output 4k.mp4
    ```
  </Card>
</CardGroup>
...
<Steps>
  <Step title="Render an existing project at 4K">
    ```bash Terminal
    npx frames render --resolution 4k --output my-video-4k.mp4
    ```
...
```bash Terminal
npx frames render --resolution 4k         # landscape 4K
npx frames render --resolution portrait-4k # vertical 4K (TikTok / Reels at max quality)
npx frames render --resolution 1080p       # explicit 1080p (no-op on 1080p compositions)
```
