...
This template-first approach means the output passes `npx @hanzo/frame lint` with zero errors on first download — your coding agent can start refining immediately without structural fixes.
...
- **In-pane preview** — scrubbing is unreliable in Claude Design's iframe sandbox. Download and use `npx @hanzo/frame preview` locally for reliable playback.
- **No linting** — Claude Design can't run `npx @hanzo/frame lint`. The template-first skeletons handle structural validity, but the self-review checklist is the only QA before download.
