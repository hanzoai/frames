...
<Note>
  **Sending your design to HeyGen as a URL ("Send to Frames"), rather than downloading a ZIP?** Use the [Send to Frames guide](https://raw.githubusercontent.com/hanzoai/frames/main/docs/guides/claude-design-send-to-frames.md) instead. That flow has different authoring rules; this page covers the Download-ZIP flow.
</Note>
...
This template-first approach means the output passes `npx frames lint` with zero errors on first download — your coding agent can start refining immediately without structural fixes.
...
- **In-pane preview** — scrubbing is unreliable in Claude Design's iframe sandbox. Download and use `npx frames preview` locally for reliable playback.
- **No linting** — Claude Design can't run `npx frames lint`. The template-first skeletons handle structural validity, but the self-review checklist is the only QA before download.
