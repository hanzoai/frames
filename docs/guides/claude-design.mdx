---
title: Claude Design
description: "Create Frames video drafts in Claude Design, then refine in any AI coding agent."
---

Claude Design produces a **valid first draft** of a Frames video — brand identity, scene content, layout, animations, and transitions. You then download the ZIP and refine in any AI coding agent (Claude Code, Cursor, Codex, Windsurf, etc.) with linting and live preview.

<Note>
  **Sending your design as a URL ("Send to Frames"), rather than downloading a ZIP?** Use the [Send to Frames guide](https://raw.githubusercontent.com/hanzoai/frames/main/docs/guides/claude-design-send-to-frames.md) instead. That flow has different authoring rules; this page covers the Download-ZIP flow.
</Note>
...
<Steps>
  <Step title="Download the instruction file">
    Open [`claude-design-frames.md`](https://github.com/hanzoai/frames/blob/main/docs/guides/claude-design-frames.md) on GitHub and click the download button (↓) to save it.
  </Step>
  ...
  <Step title="Attach the file + describe your video">
    Drag the `claude-design-frames.md` file into the chat. Describe what you want — include screenshots, brand assets, or a palette if you have them.
  </Step>
...
| --------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Claude Design               | Download [`claude-design-frames.md`](https://github.com/hanzoai/frames/blob/main/docs/guides/claude-design-frames.md) from GitHub and attach to your chat |
...
This template-first approach means the output passes `npx @hanzo/frames lint` with zero errors on first download — your coding agent can start refining immediately without structural fixes.
...
- **In-pane preview** — scrubbing is unreliable in Claude Design's iframe sandbox. Download and use `npx @hanzo/frames preview` locally for reliable playback.
- **No linting** — Claude Design can't run `npx @hanzo/frames lint`. The template-first skeletons handle structural validity, but the self-review checklist is the only QA before download.
...
<CardGroup cols={2}>
  <Card title="Prompt Guide" icon="message" href="/guides/prompting">
    More prompt patterns for Frames across Claude Code, Claude Design, and other agents.
  </Card>
  <Card title="@frames/player" icon="play" href="/packages/player">
