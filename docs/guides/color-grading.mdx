...
input endpoints `0` and `1`; Frames infers missing endpoints, and the
...
Frames supports up to four ordered selections. These are static
...
Frames ships tested shader-setting presets, not bundled LUT files:
...
npx @hanzo/frames media-treatment --capability curves --json
```
...
```bash Terminal
npx @hanzo/frames media-treatment --capability secondary --json
npx @hanzo/frames media-treatment --all --json
```
...
Frames supports project-local 3D `.cube` LUTs:
...
- Frames does not bundle third-party LUT packs.
...
- Camera LOG conversion LUTs can be loaded, but Frames does not identify
...
Frames injects exact video frames and waits for the grading runtime to
redraw before capture.

That render-parity statement applies to SDR output. In the native HDR render
path, Frames keeps HDR source pixels out of the SDR DOM capture and
