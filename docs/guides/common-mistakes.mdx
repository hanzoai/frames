...
These are mistakes that cannot be caught by the linter. For automated checks, run `npx frames lint` (see [CLI](/packages/cli#lint)).
...
<AccordionGroup>
  ...
  <Accordion title="Composition duration shorter than video">
    ...
    **After (fixed):**
    ...
    <Tip>
      A quick check: run `npx frames compositions` to see the resolved duration of each composition. If it is shorter than expected, your timeline needs extending.
    </Tip>
  </Accordion>
  ...
  <Accordion title="Missing class='clip' on timed elements">
    ...
    **After (fixed):**
    ...
    <Note>
      The linter catches this one: `npx frames lint` will flag timed elements missing `class="clip"`.
    </Note>
  </Accordion>
...
1. **Run the linter:** `npx frames lint` — catches most structural issues
