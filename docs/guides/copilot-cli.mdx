...
In agent mode, Copilot runs terminal commands (like `npx @hanzo/frame init` and `npx @hanzo/frame render`) on your behalf. Review the commands before approving them — especially if the skill pre-approves `shell` in its `allowed-tools`.
...
- **Run `npx @hanzo/frame lint` before rendering.** The linter catches structural issues the agent might miss on complex multi-scene edits.
...
- **Install registry blocks for transitions and effects.** `npx @hanzo/frame add shimmer-sweep` installs pre-built blocks, then prompt the agent to wire them in.
