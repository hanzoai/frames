...
- **Author locally**, deploy the preview + render API. Compositions are still built on your machine with `npx @hanzo/frame init`, then dropped into the template's compositions directory.
...
┌──────────────────┐       ┌────────────────────┐       ┌──────────────────────────┐
│ <frames-    │ ────▶ │ /api/render        │ ────▶ │ hanzo frame render       │
...
- **Self-hosted** rendering — see the [Rendering guide](/guides/rendering) and run `hanzo frame render --docker` on your own infrastructure.
