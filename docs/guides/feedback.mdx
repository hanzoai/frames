...
After a successful `frames render`, a short prompt may appear:
...
FRAMES_FEEDBACK_INTERVAL=5 frames render --output out.mp4
```
...
**Note:** Disabling CLI telemetry (`frames telemetry disable`) does not suppress the Studio bar — Studio feedback is gated separately. The submitted data is still sent through the same anonymous PostHog pipeline, so the same privacy guarantees apply.

### `frames feedback` command

You can submit feedback manually at any time:

```bash
# Quick rating
frames feedback --rating 10

# Rating with details
frames feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate text overlay"
```
...
```bash
frames feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
```
...
2. Publishes a minimal repro of the project and gets a public URL (the same upload as `frames publish`).
...
```
  [frames] Agent feedback: frames feedback --rating <0-10> --comment "..."
```

Agents can then submit feedback using the `frames feedback` command above.
...
The `doctor_summary` is a compact string with environment context — included automatically so you don't need to run `frames doctor` when reporting a problem:
...
frames telemetry disable

# Or via environment variable (per-session)
FRAMES_NO_TELEMETRY=1 frames render --output out.mp4

# Or via DO_NOT_TRACK (respects the global standard)
DO_NOT_TRACK=1 frames render --output out.mp4
```

Once disabled, the `frames feedback` command will print `Telemetry is disabled. Feedback not sent.` and exit without sending anything.
...
frames render --quiet --output out.mp4
```
...
- [`frames feedback`](/packages/cli#feedback) — CLI command reference
