...
| --- | --- | --- |
| **Static assets** | A frame/layer rendered to SVG/PNG/JPG/PDF, frozen under `.media/` | `frames figma asset` |
| **Brand tokens** | Figma variables/styles as composition brand variables | `frames figma tokens` (Enterprise) or the `/figma` skill via MCP (any plan) |
| **Components** | A frame as editable HTML with brand-linked colors | `frames figma component` |
...
Needed for anything you run from the `frames figma` CLI.
...
```bash
frames figma asset 'https://www.figma.com/design/KEY/Title?node-id=1-2'
```
...
```bash
frames figma component 'https://www.figma.com/design/KEY/Title?node-id=10-20'
```
