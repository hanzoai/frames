---
title: Figma Import
description: "Bring Figma designs into Frames — frozen assets, brand tokens, editable components, storyboard reconstruction, and Figma Motion timelines translated to GSAP."
---

The work your designer already did in Figma — layout, color, type, motion — becomes the starting point of a composition instead of a thing to rebuild by hand. Point at a Figma URL; the artifact lands as a native Frames piece: a frozen local file, a composition variable, editable HTML, or a paused GSAP timeline.

## What you can import

| Capability | What you get | Surface |
| --- | --- | --- |
| **Static assets** | A frame/layer rendered to SVG/PNG/JPG/PDF, frozen under `.media/` | `hanzo frame figma asset` |
| **Brand tokens** | Figma variables/styles as composition brand variables | `hanzo frame figma tokens` (Enterprise) or the `/figma` skill via MCP (any plan) |
| **Components** | A frame as editable HTML with brand-linked colors | `hanzo frame figma component` |
...
Needed for anything you run from the `hanzo frame figma` CLI.
...
```bash
hanzo frame figma asset 'https://www.figma.com/design/KEY/Title?node-id=1-2'
```
...
```bash
hanzo frame figma component 'https://www.figma.com/design/KEY/Title?node-id=10-20'
```
