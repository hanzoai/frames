---
title: Frames MCP
description: "Author, preview, and render Frames videos directly inside Claude.ai, ChatGPT, and Grok — no local install required."
---

The Frames MCP is a hosted [Model Context Protocol](https://modelcontextprotocol.io) server that lets you create, edit, preview, and render Frames video compositions from inside Claude.ai, ChatGPT, or Grok.

<Note>
  **In beta.** Features and pricing may change. Found a bug or have feedback? [File an issue on GitHub](https://github.com/hanzoai/frames/issues).
</Note>
...
The agent behind `compose` has 25+ Frames-specific skills baked in — typography, color palettes, motion principles, GSAP effects, audio-reactive animation, and captions. You don't have to specify any of this directly; describe the video you want and the agent picks the right tools.
...
### 1. Get a Hanzo account

The MCP authenticates through Hanzo IAM. Sign in or sign up at [hanzo.id](https://hanzo.id) if you don't have an account.
...
<Tabs>
  <Tab title="Claude.ai">
    <Steps>
      ...
      <Step title="Find Frames">
        Search the connector catalog for **Frames** and click **Connect**.

        Don't see it in the catalog? Add it manually: **Add custom connector**, then paste:
        ```
        https://mcp.hanzo.ai/mcp/frames
        ```
      </Step>
      <Step title="Sign in to Hanzo">
        Sign-in opens in a new window. Authorize the Frames connector against your Hanzo account.
      </Step>
...
Authors a new composition or applies an edit to an existing one. The Frames agent handles captions, blocks, layout, transitions, color, and timing internally based on your natural-language prompt.
...
### Reference your existing assets

If you've uploaded logos, fonts, or other assets to your Hanzo account, the agent can use them. Just say *"use my logo"* or *"use my brand font."* The agent resolves the asset by name and recency.
...
```bash
npx @modelcontextprotocol/inspector npx -y mcp-remote https://mcp.hanzo.ai/mcp/frames
```
...
<AccordionGroup>
  <Accordion title="OAuth keeps failing or loops">
    Verify you're using the production URL: `https://mcp.hanzo.ai/mcp/frames`.

    If sign-in completes but you see "not authorized" errors, your Hanzo account may not have access to the MCP — check your tier.
  </Accordion>
  ...
  <Accordion title="Render is stuck or never completes">
    ...
    3. If the issue persists, file an issue at [github.com/hanzoai/frames/issues](https://github.com/hanzoai/frames/issues) including the `job_id` from `render_video`
  </Accordion>

  <Accordion title='"Composition not found" error'>
    The `composition_id` is owned by the account that created it. If you're signed into a different space, you can't access compositions from another. Run `list_compositions` to see what's available from your current account.
  </Accordion>

  <Accordion title="Player widget shows blank or loads forever">
    Usually a transient connection issue between the widget and `mcp.hanzo.ai`. Refresh the chat or call `get_composition` again.
  ...
  <Accordion title="Out of credits">
    The MCP returns an error with an upgrade URL when your balance is exhausted. Visit [hanzo.ai/pricing](https://hanzo.ai/pricing) to top up.
  </Accordion>
...
For bugs or feature requests, file an issue at [github.com/hanzoai/frames/issues](https://github.com/hanzoai/frames/issues). Include:
...
- **Cloud-only rendering.** All renders run on Hanzo Cloud. Use the [CLI](/quickstart) if you need local rendering.
- **Single-user.** Each composition is owned by one Hanzo account. No team sharing in v1.
- **No binary uploads from chat.** You can reference assets you've already uploaded via the web UI, but the MCP does not currently accept new file uploads through chat. Upload first, then reference the asset by name.
...
[Frames itself is open source](https://github.com/hanzoai/frames) — the HTML composition format, CLI, renderer, and player. You can use Frames locally without the MCP.

The MCP is a Hanzo-hosted product that wraps:

- The Frames composition agent (the LLM that authors compositions)
- The cloud rendering pipeline
- The shared media catalog
...
<CardGroup cols={2}>
  <Card title="Quickstart" href="/quickstart">
    Try Frames locally with the open-source CLI.
  </Card>
