...
<Tabs>
  ...
  <Tab title="Agent / CLI">
    ...
    npx @hanzo/frames media-treatment --capability digitalGlitch --json
...
```bash Terminal
npx @hanzo/frames media-treatment --capabilities --json
npx @hanzo/frames media-treatment --capability retro-glitch --json
npx @hanzo/frames media-treatment --capability digitalGlitch --json
```
...
```bash Terminal
npx @hanzo/frames media-treatment --capability palettes --json
npx @hanzo/frames media-treatment --capability electric-ink --json
```
...
- **Two-Ink Print** is Frames' own fixed two-ink mapping. Do not describe
...
Frames evaluates combined treatment stages in one fixed order:
