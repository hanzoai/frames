...
<Steps>
  ...
  <Step title="Refine in any AI coding agent">
    ...
    npx frames preview                  # open the studio
    ```
  </Step>
</Steps>
...
- **Render still happens locally** — Open Design produces the HTML; `npx frames render` and HDR encoding still need FFmpeg + Node 22+ on your machine.
- **In-pane preview is sandboxed iframe** — full browser playback is reliable; for frame-accurate scrubbing use `npx frames preview` after handoff.
...
npx frames lint                     # verify structure
npx frames preview                  # open the studio
```
