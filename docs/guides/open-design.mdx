...
<Steps>
  ...
  <Step title="Refine in any AI coding agent">
    ...
    npx @hanzo/frame preview                  # open the studio
    ```
  </Step>
</Steps>
...
- **Render still happens locally** — Open Design produces the HTML; `npx @hanzo/frame render` and HDR encoding still need FFmpeg + Node 22+ on your machine.
- **In-pane preview is sandboxed iframe** — full browser playback is reliable; for frame-accurate scrubbing use `npx @hanzo/frame preview` after handoff.
...
npx @hanzo/frame lint                     # verify structure
npx @hanzo/frame preview                  # open the studio
```
