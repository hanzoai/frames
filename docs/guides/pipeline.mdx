...
```bash
npx @hanzo/frames capture https://example.com -o my-video/capture
```
...
| Techniques       | 2-3 picks from the [techniques library](https://github.com/hanzoai/frames/blob/main/skills/frames/references/techniques.md): SVG path drawing, Canvas 2D, CSS 3D, per-word typography, Lottie, video compositing, typing effects, variable fonts, MotionPath, velocity transitions, audio-reactive |
...
```bash
npx @hanzo/frames tts SCRIPT.md --voice af_nova --output narration.wav
npx @hanzo/frames transcribe narration.wav
```
...
Voiceover runs through [`/media-use`](/guides/prompting), which renders each line and reads its word timings back from the same audio. After generating audio, update `STORYBOARD.md` with the real beat boundaries from `transcript.json`.
...
After each composition is built, run a self-review for layout, asset placement, and animation quality. The [`/frames-core`](/guides/prompting) skill encodes the composition rules — required `class="clip"` attributes, `data-*` attribute semantics — and [`/frames-animation`](/guides/prompting) covers GSAP timeline registration and adapter registries.
...
```bash
npx @hanzo/frames lint                              # static HTML structure checks
npx @hanzo/frames check                             # one browser session: runtime errors, layout, motion, contrast
npx @hanzo/frames snapshot my-video --at 2.9,10.4   # PNGs at beat midpoints
```
...
The pipeline delivers the localhost Studio URL as the handoff. Your AI agent runs `npx @hanzo/frames preview` and shares the project URL. Rendering to MP4 is on-demand:

```bash
npx @hanzo/frames render --output my-video.mp4
```
...
- For surgical tweaks, open a composition file directly (e.g. `compositions/beat-3-proof.html`) and adjust animations, colors, or layout. `npx @hanzo/frames preview` shows changes live.
...
- Shipping a product launch. Most of the [launch videos](/launch-videos) use this artifact layout.
