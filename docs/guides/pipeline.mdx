...
```bash
npx frames capture https://example.com -o my-video/capture
```
...
```bash
npx frames tts SCRIPT.md --voice af_nova --output narration.wav
npx frames transcribe narration.wav
```
...
```bash
npx frames lint                              # static HTML structure checks
npx frames check                             # one browser session: runtime errors, layout, motion, contrast
npx frames snapshot my-video --at 2.9,10.4   # PNGs at beat midpoints
```
...
The pipeline delivers the localhost Studio URL as the handoff. Your AI agent runs `npx frames preview` and shares the project URL. Rendering to MP4 is on-demand:
...
- For surgical tweaks, open a composition file directly (e.g. `compositions/beat-3-proof.html`) and adjust animations, colors, or layout. `npx frames preview` shows changes live.
