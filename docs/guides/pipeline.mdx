...
```bash
npx @hanzo/frame capture https://example.com -o my-video/capture
```
...
```bash
npx @hanzo/frame tts SCRIPT.md --voice af_nova --output narration.wav
npx @hanzo/frame transcribe narration.wav
```
...
```bash
npx @hanzo/frame lint                              # static HTML structure checks
npx @hanzo/frame check                             # one browser session: runtime errors, layout, motion, contrast
npx @hanzo/frame snapshot my-video --at 2.9,10.4   # PNGs at beat midpoints
```
...
The pipeline delivers the localhost Studio URL as the handoff. Your AI agent runs `npx @hanzo/frame preview` and shares the project URL. Rendering to MP4 is on-demand:

```bash
npx @hanzo/frame render --output my-video.mp4
```
...
- For surgical tweaks, open a composition file directly (e.g. `compositions/beat-3-proof.html`) and adjust animations, colors, or layout. `npx @hanzo/frame preview` shows changes live.
