...
| `/frames-registry` | Block and component installation via `hanzo frame add`                     |
...
Claude Design produces a valid first draft (brand identity, scene content, animations, transitions). Download the ZIP and refine in any AI coding agent with `npx @hanzo/frame preview` running. See the [Claude Design guide](/guides/claude-design) for the full workflow.
...
- **Don't paste long error logs into the prompt without context.** Run `npx @hanzo/frame check` first — lint catches structural issues, validate catches runtime errors (JS exceptions, missing assets, contrast problems).
...
1. `npx @hanzo/frame init my-video` — scaffold a project (skills install automatically)
2. Open the project in Claude Code (or Cursor / Codex)
3. Prompt with `/frames` and one of the shapes above
4. `npx @hanzo/frame preview` — watch in the browser as the agent edits
5. Iterate with small targeted prompts
6. `npx @hanzo/frame render --output final.mp4` when you're happy
