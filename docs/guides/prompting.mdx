...
```bash
npx skills add hanzoai/frames
```

The installer shows a picker. Select the **core skills** below — every project needs them. In Claude Code, restart the session after installing; the skills register as **slash commands**. Start at `/frames`: it orients you to the whole surface and routes "make me a video" requests to the right workflow.

**Core skills — install all of these**

| Slash command           | What it loads                                                              |
| ----------------------- | -------------------------------------------------------------------------- |
| `/frames`          | **Read first.** The entry skill — capability map + video router; sends "make me a video" intent to the right workflow |
| `/frames-core`     | Composition contract — HTML structure, `data-*` attributes, clips, tracks  |
| `/frames-animation`| All animation — motion rules, scene blueprints, transitions, and the runtime adapters (GSAP, Lottie, Three.js, Anime.js, CSS, WAAPI, TypeGPU) |
| `/frames-creative` | Creative direction — design spec, palettes, typography, narration, beats   |
| `/frames-cli`      | Dev-loop CLI — `init`, `lint`, `validate`, `inspect`, `preview`, `render`, `doctor` |
| `/media-use`    | Asset preprocessing — `tts`, `transcribe`, `remove-background`             |
| `/frames-registry` | Block and component installation via `frames add`                     |
| `/general-video`        | The general authoring workflow — fallback for any video that doesn't match a specific workflow below |

**Optional workflows — add the ones that match your inputs** (`/frames` routes to whichever you've installed)
...
| `/remotion-to-frames` | Port an existing Remotion (React) composition to Frames HTML         |

<Tip>
  To skip the picker and install everything (core + every workflow) in one shot, run `npx skills add hanzoai/frames --all`. And start Hyperframes prompts with `/frames` (or invoke the skill another way for non-Claude agents) — it loads the routing + composition context explicitly so the agent picks the right workflow and gets the rules right the first time.
</Tip>

## Claude Design

Claude Design uses a different setup. Download [`claude-design-frames.md`](https://github.com/hanzoai/frames/blob/main/docs/guides/claude-design-frames.md) from GitHub (click the ↓ button), then **attach it to your chat** (don't paste the URL — file attachments produce better output):
...
Claude Design produces a valid first draft (brand identity, scene content, animations, transitions). Download the ZIP and refine in any AI coding agent with `npx frames preview` running. See the [Claude Design guide](/guides/claude-design) for the full workflow.
...
> Using `/frames`, create a 10-second product intro with a fade-in title over a dark background and subtle background music.

> Make a 9:16 TikTok-style hook video about [topic] using `/frames`, with bouncy captions synced to a TTS narration.
...
> Take a look at this GitHub repo https://github.com/hanzoai/frames and explain its uses and architecture to me using `/frames`.

> Summarize the attached PDF into a 45-second pitch video using `/frames`.

> Read this changelog and turn the top three changes into a 30-second release announcement video using `/frames`.

> Turn this CSV into an animated bar chart race using `/frames`.
...
Frames supports three TTS providers: **HeyGen** (Starfish voices, requires sign-in via `npx frames auth`), **ElevenLabs** (requires API key), and **Kokoro** (free, runs locally, no API key needed). The agent asks which provider to use — or picks automatically in autonomous mode. Describe the content and the agent picks a voice, or request one directly:
...
- **Don't skip the slash command.** Without `/frames`, the agent may guess at HTML video conventions instead of using the framework's actual rules (`class="clip"` on timed elements, `window.__timelines` registration, etc.).
- **Don't paste long error logs into the prompt without context.** Run `npx frames check` first — lint catches structural issues, validate catches runtime errors (JS exceptions, missing assets, contrast problems).
...
1. `npx frames init my-video` — scaffold a project (skills install automatically)
2. Open the project in Claude Code (or Cursor / Codex)
3. Prompt with `/frames` and one of the shapes above
4. `npx frames preview` — watch in the browser as the agent edits
5. Iterate with small targeted prompts
6. `npx frames render --output final.mp4` when you're happy
