...
<Steps>
  <Step title="Verify ffmpeg is installed">
    ...
    Confirm with `npx @hanzo/frames doctor` — both should be green.
  </Step>
  <Step title="Remove the background from your video">
    ```bash Terminal
    npx @hanzo/frames remove-background subject.mp4 -o transparent.webm
    ```
  ...
  <Step title="Drop it into a composition">
    ...
    Render the composition with the usual `frames render`.
  </Step>
</Steps>
...
```bash Terminal
npx @hanzo/frames remove-background subject.mp4 -o transparent.webm        # web playback
npx @hanzo/frames remove-background subject.mp4 -o transparent.mov         # editing
npx @hanzo/frames remove-background portrait.jpg -o cutout.png       # still image
```
...
```bash Terminal
npx @hanzo/frames remove-background subject.mp4 \
  -o subject.webm \
...
- **Opt into CUDA** by setting `FRAMES_CUDA=1` and providing a GPU-enabled `onnxruntime-node` build (the bundled build is CPU + CoreML only, to keep the install small for the 99% of users who don't have a GPU):
  ...
  ```bash Terminal
  FRAMES_CUDA=1 npx @hanzo/frames remove-background subject.mp4 -o transparent.webm --device cuda
  ```

Run `npx @hanzo/frames remove-background --info` to see what providers are detected on your machine and which one `auto` would pick.
...
- **Person / portrait video, web playback, MIT-clean** → use the built-in `frames remove-background` (this is what it's tuned for).
...
The pipeline shells out to ffmpeg for decode + encode. Install via `brew install ffmpeg` on macOS or `sudo apt install ffmpeg` on Debian/Ubuntu. Verify with `npx @hanzo/frames doctor`.
...
- CLI: [`frames remove-background`](/packages/cli#remove-background)
