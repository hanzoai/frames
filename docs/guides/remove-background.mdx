...
Background removal — also called *matting* in VFX — separates a foreground subject (typically a person) from its background. The output is a video with an alpha channel: fully transparent where the background was, opaque where the subject is. Drop it into any Frames composition as a `<video>` tag and the subject floats over whatever you put behind them.
...
<Steps>
  <Step title="Verify ffmpeg is installed">
    ...
    Confirm with `npx @hanzo/frame doctor` — both should be green.
  </Step>
  <Step title="Remove the background from your video">
    ```bash Terminal
    npx @hanzo/frame remove-background subject.mp4 -o transparent.webm
    ```

    On the first run, the CLI downloads ~168 MB of model weights to `~/.cache/frames/background-removal/models/`. Subsequent runs reuse the cache.
  ...
  <Step title="Drop it into a composition">
    ...
    Render the composition with the usual `hanzo frame render`.
  </Step>
</Steps>
...
```bash Terminal
npx @hanzo/frame remove-background subject.mp4 -o transparent.webm        # web playback
npx @hanzo/frame remove-background subject.mp4 -o transparent.mov         # editing
npx @hanzo/frame remove-background portrait.jpg -o cutout.png       # still image
```
...
```bash Terminal
npx @hanzo/frame remove-background subject.mp4 \
  -o subject.webm \
...
Real-world numbers from running u²-net_human_seg on a 4-second 1080p clip:
...
- **Opt into CUDA** by setting `FRAMES_CUDA=1` and providing a GPU-enabled `onnxruntime-node` build (the bundled build is CPU + CoreML only, to keep the install small for the 99% of users who don't have a GPU):

  ```bash Terminal
  FRAMES_CUDA=1 npx @hanzo/frame remove-background subject.mp4 -o transparent.webm --device cuda
  ```

Run `npx @hanzo/frame remove-background --info` to see what providers are detected on your machine and which one `auto` would pick.
...
The CLI ships **one model on purpose** — the one that's MIT-licensed, runs everywhere, and produces production-quality output for person/portrait video. The list below leads with **free, open-source tools** that pair naturally with Frames. Each entry calls out the actual catch — license, install effort, hardware needs — so you can pick the right one for your situation. Full benchmarks come from the same 4-second 1080p matting comparison.
...
After running any of these externally, encode the output as a Frames-compatible transparent WebM with:
...
- **Person / portrait video, web playback, MIT-clean** → use the built-in `hanzo frame remove-background` (this is what it's tuned for).
...
mkdir -p ~/.cache/frames/background-removal/models
curl -L -o ~/.cache/frames/background-removal/models/u2net_human_seg.onnx \
...
The pipeline shells out to ffmpeg for decode + encode. Install via `brew install ffmpeg` on macOS or `sudo apt install ffmpeg` on Debian/Ubuntu. Verify with `npx @hanzo/frame doctor`.
...
- CLI: [`hanzo frame remove-background`](/packages/cli#remove-background)
- Eval: matting comparison v7, u²-net_human_seg on a 4-second 1080p clip
