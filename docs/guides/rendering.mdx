...
npx @hanzo/frame render --crf 15 --output pristine.mp4

# Target a specific bitrate (useful for size-constrained delivery)
npx @hanzo/frame render --video-bitrate 10M --output controlled.mp4
```
...
```bash Terminal
npx @hanzo/frame render --format gif --fps 15 --gif-loop 0 --output demo.gif
```
...
npx @hanzo/frame render --gpu --output encoded-fast.mp4

# Opt out of hardware Chrome/WebGL capture
npx @hanzo/frame render --no-browser-gpu --output software-browser.mp4
...
npx @hanzo/frame render --workers 1 --output output.mp4

# Let Hyperframes pick based on your CPU
npx @hanzo/frame render --workers auto --output output.mp4

# Maximum parallelism (use with caution on laptops)
npx @hanzo/frame render --workers 24 --output output.mp4
```
...
```bash Terminal
# CLI flag
npx @hanzo/frame render --max-concurrent-renders 2 --output output.mp4
...
```bash Terminal
npx @hanzo/frame render --format mov --output overlay.mov
```
...
```bash Terminal
npx @hanzo/frame render --format png-sequence --output frames/
```
...
- Use `npx @hanzo/frame benchmark` to find optimal settings for your system
...
<CardGroup cols={2}>
  ...
  <Card title="Cloud Rendering" icon="cloud" href="/deploy/cloud">
    Render on the hosted cloud — no local Chrome or FFmpeg
  </Card>
