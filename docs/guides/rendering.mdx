...
npx frames render --crf 15 --output pristine.mp4

# Target a specific bitrate (useful for size-constrained delivery)
npx frames render --video-bitrate 10M --output controlled.mp4
```
...
```bash Terminal
npx frames render --format gif --fps 15 --gif-loop 0 --output demo.gif
```
...
npx frames render --gpu --output encoded-fast.mp4

# Opt out of hardware Chrome/WebGL capture
npx frames render --no-browser-gpu --output software-browser.mp4
...
npx frames render --workers 1 --output output.mp4

# Let Hyperframes pick based on your CPU
npx frames render --workers auto --output output.mp4

# Maximum parallelism (use with caution on laptops)
npx frames render --workers 24 --output output.mp4
```
...
```bash Terminal
# CLI flag
npx frames render --max-concurrent-renders 2 --output output.mp4
...
```bash Terminal
npx frames render --format mov --output overlay.mov
```
...
```bash Terminal
npx frames render --format png-sequence --output frames/
```
...
- Use `npx frames benchmark` to find optimal settings for your system
