...
<AccordionGroup>
  <Accordion title='"No composition found"'>
    ...
    **Fix:** Run `npx @hanzo/frame init` to create a composition from an [example](/examples), or verify your `index.html` has the correct structure:
  ...
  <Accordion title='"FFmpeg not found"'>
    ...
    After installing, run `npx @hanzo/frame doctor` to verify the CLI can find it.

    <Tip>
      If you cannot install FFmpeg, use [Docker mode](/guides/rendering) instead — it bundles FFmpeg inside the container: `npx @hanzo/frame render --docker --output output.mp4`
    </Tip>
  </Accordion>

  <Accordion title="Lint errors">
    Run `npx @hanzo/frame lint` to check for common structural issues (see [CLI: lint](/packages/cli#lint)):
  ...
  <Accordion title="Preview not updating">
    ...
    2. Stop and restart `npx @hanzo/frame preview`
  ...
  <Accordion title="Preview stutters or plays at a low frame rate">
    ...
    **Temporary workaround:** render to mp4 and watch the output. Render is accurate regardless of per-frame cost.
    ...
    ```bash Terminal
    npx @hanzo/frame render --quality draft --output preview.mp4
    ```
  ...
  <Accordion title="Render is slow">
    ...
    2. Run `npx @hanzo/frame benchmark` to find the optimal worker count for your system
  ...
  <Accordion title="Video asset plays black in preview but renders fine (HEVC/H.265)">
    ...
    1. Confirm auto-proxying is not disabled: no `--no-proxy` flag, and `media.autoProxy` is not `false` in `frames.json`.
    2. Run `npx @hanzo/frame doctor` to confirm ffmpeg/ffprobe are installed; the proxy transcode needs both.
    3. Run `npx @hanzo/frame lint --verbose`: the info-level `hevc_preview_codec` finding names every affected asset.
...
Run `npx @hanzo/frame doctor` to check your environment:
...
1. Run `npx @hanzo/frame info` to gather system and project details
2. Check [GitHub Issues](https://github.com/hanzoai/frames/issues) for similar reports
3. Open a new issue with the output of `npx @hanzo/frame info` and steps to reproduce
