...
```bash
npx @hanzo/frames init my-video --example blank
cd my-video
...
npx @hanzo/frames render --quality standard --output review.mp4
```

For fast iteration renders, use draft quality:

```bash
npx @hanzo/frames render --quality draft --output draft.mp4
```

For final delivery:

```bash
npx @hanzo/frames render --quality high --fps 30 --output final.mp4
```
...
| Command | Use it for |
| --- | --- |
| `npx @hanzo/frames init my-video` | Create a new project |
| `npx @hanzo/frames init my-video --example warm-grain` | Start from a visual template |
| `npx @hanzo/frames init my-video --video source.mp4` | Import video and generate captions from the source audio |
| `npx @hanzo/frames capture https://example.com` | Capture a website as source material for a video |
| `npx @hanzo/frames preview` | Open the live Studio preview |
| `npx @hanzo/frames lint` | Catch structural mistakes before preview or render |
| `npx @hanzo/frames check` | Run the composition in headless Chrome to catch runtime errors |
| `npx @hanzo/frames check` | Find text overflow and layout problems across the timeline |
| `npx @hanzo/frames snapshot --at 1,3,5` | Save PNG checks at exact timestamps |
| `npx @hanzo/frames render --output final.mp4` | Render the video |
| `npx @hanzo/frames publish` | Upload the project and get a shareable Frames URL |
| `npx @hanzo/frames doctor` | Check Node.js, FFmpeg, Chrome, Docker, and other dependencies |
| `npx @hanzo/frames docs` | Open local CLI docs |
| `npx @hanzo/frames upgrade` | Check for a newer CLI version |
...
| Goal | Command |
| --- | --- |
| Fast iteration | `npx @hanzo/frames render --quality draft --output draft.mp4` |
| Review link | `npx @hanzo/frames render --quality standard --output review.mp4` |
| Final export | `npx @hanzo/frames render --quality high --fps 30 --output final.mp4` |
| Transparent overlay | `npx @hanzo/frames render --format webm --output overlay.webm` |
| Deterministic output | `npx @hanzo/frames render --docker --output final.mp4` |
...
```bash
npx @hanzo/frames publish ./my-video   # publish a specific folder
npx @hanzo/frames publish --yes        # skip the confirmation prompt in scripts
```
...
For editor-facing changes, keep `npx @hanzo/frames preview` running, then have the agent use `agent-browser` against the local preview URL.

## Quick Fixes

| Problem | Command or check |
| --- | --- |
| Preview will not start | `npx @hanzo/frames doctor` |
| Port already in use | `npx @hanzo/frames preview --port 4567` |
| Render fails | `npx @hanzo/frames lint` then `npx @hanzo/frames check` |
| Need exact frame checks | `npx @hanzo/frames snapshot --at 1,2.5,5` |
| Text overflows in the frame | `npx @hanzo/frames check` |
| Final render is too slow | Try `--quality draft`, reduce image sizes, or lower `--fps` |
| Need to share editable project | `npx @hanzo/frames publish` |
