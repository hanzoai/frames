...
description: "Fast reference for video editors and creative people directing agents, cutting timing, tweaking layouts, previewing, and publishing Frames projects."
---

Use this as a fast reference when you are directing agents, cutting timing, making visual layout tweaks, previewing, and sharing Frames projects.

## The Fast Loop

```bash
npx @hanzo/frame init my-video --example blank
cd my-video
...
npx @hanzo/frame render --quality standard --output review.mp4
```

For fast iteration renders, use draft quality:

```bash
npx @hanzo/frame render --quality draft --output draft.mp4
```

For final delivery:

```bash
npx @hanzo/frame render --quality high --fps 30 --output final.mp4
```
...
Run Frames commands from the project root, where `index.html` lives. If you are not sure where you are, run `pwd` then `ls`. If you see `index.html`, you are in the right place.
...
| Command | Use it for |
| --- | --- |
| `npx @hanzo/frame init my-video` | Create a new project |
| `npx @hanzo/frame init my-video --example warm-grain` | Start from a visual template |
| `npx @hanzo/frame init my-video --video source.mp4` | Import video and generate captions from the source audio |
| `npx @hanzo/frame capture https://example.com` | Capture a website as source material for a video |
| `npx @hanzo/frame preview` | Open the live Studio preview |
| `npx @hanzo/frame lint` | Catch structural mistakes before preview or render |
| `npx @hanzo/frame check` | Run the composition in headless Chrome to catch runtime errors |
| `npx @hanzo/frame check` | Find text overflow and layout problems across the timeline |
| `npx @hanzo/frame snapshot --at 1,3,5` | Save PNG checks at exact timestamps |
| `npx @hanzo/frame render --output final.mp4` | Render the video |
| `npx @hanzo/frame publish` | Upload the project and get a shareable Frames URL |
| `npx @hanzo/frame doctor` | Check Node.js, FFmpeg, Chrome, Docker, and other dependencies |
| `npx @hanzo/frame docs` | Open local CLI docs |
| `npx @hanzo/frame upgrade` | Check for a newer CLI version |
...
| Goal | Command |
| --- | --- |
| Fast iteration | `npx @hanzo/frame render --quality draft --output draft.mp4` |
| Review link | `npx @hanzo/frame render --quality standard --output review.mp4` |
| Final export | `npx @hanzo/frame render --quality high --fps 30 --output final.mp4` |
| Transparent overlay | `npx @hanzo/frame render --format webm --output overlay.webm` |
| Deterministic output | `npx @hanzo/frame render --docker --output final.mp4` |
...
Publish zips the current project, uploads it, and prints a stable `frames.hanzo.ai` URL. The URL includes a claim token so the recipient can open it, claim the project, and continue editing in the web app.

```bash
npx @hanzo/frame publish ./my-video   # publish a specific folder
npx @hanzo/frame publish --yes        # skip the confirmation prompt in scripts
```
...
Use it when you want the agent to open the Frames Studio preview, play or scrub the video, click timeline controls, inspect visible UI text, capture screenshots for review, or record proof of a tested flow.

For editor-facing changes, keep `npx @hanzo/frame preview` running, then have the agent use `agent-browser` against the local preview URL.

## Quick Fixes

| Problem | Command or check |
| --- | --- |
| Preview will not start | `npx @hanzo/frame doctor` |
| Port already in use | `npx @hanzo/frame preview --port 4567` |
| Render fails | `npx @hanzo/frame lint` then `npx @hanzo/frame check` |
| Need exact frame checks | `npx @hanzo/frame snapshot --at 1,2.5,5` |
| Text overflows in the frame | `npx @hanzo/frame check` |
| Final render is too slow | Try `--quality draft`, reduce image sizes, or lower `--fps` |
| Need to share editable project | `npx @hanzo/frame publish` |
