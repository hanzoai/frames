...
```bash
npx frames init my-video --example blank
cd my-video
npx frames preview
```
...
npx frames lint
npx frames check
npx frames render --quality standard --output review.mp4
```

For fast iteration renders, use draft quality:

```bash
npx frames render --quality draft --output draft.mp4
```

For final delivery:

```bash
npx frames render --quality high --fps 30 --output final.mp4
```
...
| Command | Use it for |
| --- | --- |
| `npx frames init my-video` | Create a new project |
| `npx frames init my-video --example warm-grain` | Start from a visual template |
| `npx frames init my-video --video source.mp4` | Import video and generate captions from the source audio |
| `npx frames capture https://example.com` | Capture a website as source material for a video |
| `npx frames preview` | Open the live Studio preview |
| `npx frames lint` | Catch structural mistakes before preview or render |
| `npx frames check` | Run the composition in headless Chrome to catch runtime errors |
| `npx frames check` | Find text overflow and layout problems across the timeline |
| `npx frames snapshot --at 1,3,5` | Save PNG checks at exact timestamps |
| `npx frames render --output final.mp4` | Render the video |
| `npx frames publish` | Upload the project and get a shareable Frames URL |
| `npx frames doctor` | Check Node.js, FFmpeg, Chrome, Docker, and other dependencies |
| `npx frames docs` | Open local CLI docs |
| `npx frames upgrade` | Check for a newer CLI version |
...
| Goal | Command |
| --- | --- |
| Fast iteration | `npx frames render --quality draft --output draft.mp4` |
| Review link | `npx frames render --quality standard --output review.mp4` |
| Final export | `npx frames render --quality high --fps 30 --output final.mp4` |
| Transparent overlay | `npx frames render --format webm --output overlay.webm` |
| Deterministic output | `npx frames render --docker --output final.mp4` |
...
```bash
npx frames publish ./my-video   # publish a specific folder
npx frames publish --yes        # skip the confirmation prompt in scripts
```
...
For editor-facing changes, keep `npx frames preview` running, then have the agent use `agent-browser` against the local preview URL.

## Quick Fixes

| Problem | Command or check |
| --- | --- |
| Preview will not start | `npx frames doctor` |
| Port already in use | `npx frames preview --port 4567` |
| Render fails | `npx frames lint` then `npx frames check` |
| Need exact frame checks | `npx frames snapshot --at 1,2.5,5` |
| Text overflows in the frame | `npx frames check` |
| Final render is too slow | Try `--quality draft`, reduce image sizes, or lower `--fps` |
| Need to share editable project | `npx frames publish` |
