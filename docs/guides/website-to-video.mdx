...
<Note>
  You don't need to run `npx frames capture` manually — the skill instructs the agent to capture as the first step. The capture command is documented [below](#capture-command) for advanced use.
</Note>
...
<AccordionGroup>
  <Accordion title="Capture times out">
    ...
    ```bash
    npx frames capture https://example.com --timeout 180000
    ```
  </Accordion>
