...
<Steps>
  <Step title="Install skills">
    Skills teach your AI agent how to capture websites and create Frames compositions. Install once — they persist across sessions.
...
<Note>
  You don't need to run `npx @hanzo/frames capture` manually — the skill instructs the agent to capture as the first step. The capture command is documented [below](#capture-command) for advanced use.
</Note>
...
OpenRouter is used when its key is present (it takes priority if both are set). The default model is `google/gemini-3.1-flash-lite`; override it with `FRAMES_OPENROUTER_MODEL` (any vision-capable OpenRouter model), just as `FRAMES_GEMINI_MODEL` overrides the Gemini default.
...
<AccordionGroup>
  <Accordion title="Capture times out">
    ...
    ```bash
    npx @hanzo/frames capture https://example.com --timeout 180000
    ```
  </Accordion>
