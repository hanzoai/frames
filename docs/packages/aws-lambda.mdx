---
title: "@frames/aws-lambda"
description: "AWS Lambda and Step Functions adapter for distributed Frames rendering."
---

The AWS Lambda package runs Frames' distributed render primitives on AWS. It ships the Lambda handler, a Node client SDK, and an optional CDK construct for provisioning the render stack inside an adopter's AWS account.

```bash
npm install @frames/aws-lambda
```

## When to Use

**Use `@frames/aws-lambda` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@frames/aws-lambda` | Handler types, runtime helpers, S3 transport, and client SDK exports |
| `@frames/aws-lambda/handler` | Lambda handler entry point for Step Functions dispatch |
| `@frames/aws-lambda/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
| `@frames/aws-lambda/cdk` | Optional CDK construct for provisioning the render bucket, Lambda, and Step Functions stack |

`aws-cdk-lib` and `constructs` are optional peer dependencies. SDK-only consumers do not need them unless they import from `@frames/aws-lambda/cdk`.
...
```typescript
import { deploySite, getRenderProgress, renderToLambda } from "@frames/aws-lambda/sdk";
...
const handle = await renderToLambda({
  ...
  stateMachineArn: "arn:aws:states:us-east-1:123456789012:stateMachine:frames-render",
  config: {
    fps: 30,
    width: 1920,
    height: 1080,
