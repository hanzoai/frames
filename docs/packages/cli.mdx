...
<Tabs>
  <Tab title="Create">
    ...
    Trying `add` with an example's name (e.g. `frames add warm-grain`) emits a clear error pointing you at `init --example`.
    ...
    **Supported transcript formats:**
    ...
    | Format | Source |
    |--------|--------|
    | whisper.cpp JSON | `frames init --video`, `frames transcribe` |
...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
  ...
  <Tab title="Utilities">
    ...
    **Using `frames skills` is already fine** — as of v0.4.5 the CLI sets `GIT_CLONE_PROTECTION_ACTIVE=0` on the child environment, which is the opt-in knob Git provides for exactly this case. You don't need to do anything.
...
```bash
frames auth status
frames auth status --json   # machine-readable
```
...
```bash
frames auth login                          # one-time
frames cloud render ./my-video             # zip + upload + poll + download
frames cloud render ./my-video --no-wait   # submit and exit with the render_id
frames cloud list                          # browse recent renders
```
...
Use `frames cloud render --dry-run` to inspect the compressed size and largest included files without authenticating, uploading, or starting a render. Project-specific `.framesignore` rules use gitignore syntax and also apply to `frames publish`; keep rules narrow because dynamically selected assets may not have obvious static references.

Render parameters mirror the local `frames render` UX where they overlap:
...
frames cloud render --asset-id asst_abc123
...
```bash
frames cloud list
frames cloud list --limit 50 --json
frames cloud list --all
```
...
```bash
frames cloud get hfr_abc123
frames cloud get hfr_abc123 --json
```
...
```bash
frames cloud delete hfr_abc123
frames cloud delete hfr_abc123 --no-confirm --json
```

### When to pick `cloud` vs `lambda` vs local render

- `frames render` (local): fastest iteration loop. Use during composition authoring.
- `frames lambda render`: bring-your-own-AWS distributed rendering. Use when you've already invested in AWS and want chunked parallelism on your own account.
- `frames cloud render`: zero-infra option. Hanzo Cloud runs the render, metered against the org's balance. Use when you don't want to manage Chrome/ffmpeg/AWS locally.

### Auth + base URL

`cloud` reuses the credential resolved by `frames auth status`. Override the API base for staging tests with `HANZO_BASE_URL` (default `https://api.hanzo.ai`).

## frames lambda

Deploy Frames distributed rendering to AWS Lambda and drive renders from your laptop or CI.

The `frames lambda` command group wraps the `@frames/aws-lambda` SDK plus AWS SAM so an end-to-end render is three commands:

```bash
frames lambda deploy
frames lambda render ./my-project --width 1920 --height 1080 --wait
frames lambda destroy   # when you're done
```
...
frames lambda render ./my-project --site-id=abc1234deadbeef0 --width 1920 --height 1080
```
...
```bash
frames lambda render ./my-project \
...
The composition can be parameterised with `--variables` / `--variables-file`, mirroring the local `frames render` flags. Variables flow into the Step Functions execution input and reach every chunk worker as `window.__hfVariables`. Mismatches against the composition's `data-composition-variables` declaration print as warnings; pass `--strict-variables` to fail the command instead.

```bash
frames lambda render ./my-template --site-id=abc1234deadbeef0 \
...
```bash
frames lambda render-batch ./my-template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10
```
...
Pass `--json` for the machine-readable form. Poll each execution via `frames lambda progress <renderId>` (or use the returned `executionArn`).
...
```bash
frames lambda render-batch ./my-template --batch ./users.jsonl \
...
```bash
frames lambda progress hf-render-abcd1234
```
...
`frames lambda` keeps per-stack metadata under `<cwd>/.frames/lambda-stack-<name>.json` so the verbs don't need to call `describe-stacks` every time. Commit the file to a repo or `.gitignore` it depending on your workflow — it contains the bucket name, state-machine ARN, and region, none of which are secrets but all of which are AWS-account-identifying.

## frames cloudrun

The Google Cloud counterpart to `frames lambda`. Deploys Frames distributed rendering to Cloud Run + Cloud Workflows and drives renders from your laptop or CI. Wraps the `@frames/gcp-cloud-run` SDK plus `terraform` (the module shipped with the package) and `gcloud` / Cloud Build for the image.

```bash
frames cloudrun deploy --project my-gcp-project
frames cloudrun render ./my-project --width 1920 --height 1080 --wait
frames cloudrun destroy --project my-gcp-project   # when you're done
```
...
```bash
frames cloudrun deploy --project my-gcp-project --region us-central1
frames cloudrun deploy --project my-gcp-project --image us-central1-docker.pkg.dev/my-gcp-project/frames/frames-render:v1
```
...
`frames cloudrun` caches the deployed stack's coordinates under `~/.frames/cloudrun-state.json` (project id, region, bucket, service URL, workflow id) so `render` / `progress` / `destroy` don't need them re-passed. None are secrets, but all are GCP-project-identifying.

## frames.json

`frames init` writes a `frames.json` file at the root of every new project. `frames add` reads it to know which registry to pull items from and where to drop them. Edit the file (or delete it to fall back to defaults) to reshape your project layout or point at a custom registry.
...
<CardGroup cols={2}>
  ...
  <Card title="Studio" icon="palette" href="/packages/studio">
    The editor UI that powers `frames preview`. Use directly to embed in your own app.
  </Card>
