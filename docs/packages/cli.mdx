...
<Tabs>
  <Tab title="Agent mode (default)">
    ...
    npx frames init my-video --example blank --video video.mp4
    npx frames render --output output.mp4 --fps 30 --quality standard
    npx frames upgrade --check --json
    ```
  </Tab>
...
<Steps>
  <Step title="Create a project">
    ...
    ```bash
    npx frames init --example warm-grain
    ```
    You will be prompted for a project name, or pass it as an argument:
    ```bash
    npx frames init my-video --example warm-grain
    ```
  ...
  <Step title="Render to MP4">
    ...
    ```bash
    npx frames render --output output.mp4
    ```
    Render a specific composition instead of `index.html`:
    ```bash
    npx frames render -c compositions/intro.html -o intro.mp4
    ```
    For deterministic output, add `--docker`:
    ```bash
    npx frames render --docker --output output.mp4
    ```
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Create">
    ...
    npx frames init my-video --example blank --tailwind
    ...
    npx frames add claude-code-window

    # Add a component (effect / snippet)
    npx frames add shader-wipe

    # Target a different project dir
    npx frames add shader-wipe --dir ./my-video

    # Headless / CI (skip clipboard; also: --json for a machine-readable result)
    npx frames add shader-wipe --no-clipboard --json
    ```
    ...
    npx frames transcribe audio.mp3
    npx frames transcribe video.mp4 --model medium.en --language en

    # Import existing transcripts from other tools
    npx frames transcribe subtitles.srt
    npx frames transcribe captions.vtt
    npx frames transcribe openai-response.json

    # Export caption sidecars from transcript.json
    npx frames transcribe transcript.json --to srt
    ```
...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
    ...
    npx frames tts "Welcome to Frames"

    # Choose a voice
    npx frames tts "Hello world" --voice am_adam

    # Save to a specific file
    npx frames tts "Intro" --voice bf_emma --output narration.wav

    # Adjust speech speed
    npx frames tts "Slow and clear" --speed 0.8

    # Generate Spanish speech (lang auto-detected from the `e` voice prefix)
    npx frames tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav

    # Override the phonemizer (read English text with a French voice)
    npx frames tts "Bonjour le monde" --voice af_heart --lang fr-fr

    # Read text from a file
    npx frames tts script.txt
    ...
    npx frames remove-background avatar.mp4 -o transparent.webm

    # ProRes 4444 .mov for editing round-trip
    npx frames remove-background avatar.mp4 -o transparent.mov

    # Single image → transparent PNG
    npx frames remove-background portrait.jpg -o cutout.png

    # Layer separation: cutout AND inverse-alpha background plate in one pass
    npx frames remove-background avatar.mp4 \
      -o subject.webm --background-output plate.webm

    # Force CPU on a machine that has CoreML or CUDA
    npx frames remove-background avatar.mp4 -o transparent.webm --device cpu

    # Inspect detected providers without rendering
    npx frames remove-background --info
    ```
    ...
    ```bash
    npx frames capture https://stripe.com
    npx frames capture https://linear.app -o linear-capture
    npx frames capture https://example.com --json
    ```
  ...
  <Tab title="Preview">
    ...
    ```bash
    npx frames publish [dir]
    npx frames publish --yes
    ```
    ...
    ```bash
    npx frames lint [dir]
    npx frames lint [dir] --verbose   # include info-level findings
    npx frames lint [dir] --json      # machine-readable JSON output
    ```
    ```
    ◆  Linting my-project/index.html
    ...
    **Severity levels:**
    ...
    ```bash
    npx frames check [dir]
    npx frames check [dir] --json         # {ok, lint, runtime, layout, motion, contrast, snapshots}
    npx frames check [dir] --snapshots    # annotated overview frames + per-finding crops
    npx frames check [dir] --at 1.5,4,7.25
    npx frames check [dir] --strict       # exit non-zero on warnings too
    ```
    ...
    ```bash
    npx frames beats [dir]
    npx frames beats [dir] --json   # machine-readable JSON output
    ```
    ...
    Requires a local Chrome (the same one used by `render`; run `npx frames browser ensure` if missing). Detection runs the **same** algorithm the Studio uses; results are near-identical (a different headless-Chrome audio sample rate can shift beat times by a frame or two).
    ...
    ```bash
    npx frames inspect [dir]
    npx frames inspect [dir] --json
    npx frames inspect [dir] --samples 15
    npx frames inspect [dir] --at 1.5,4,7.25
    ```
    ...
    ```bash
    npx frames snapshot my-project --at 2.9,10.4,18.7
    npx frames snapshot my-project --frames 10
    ```
  ...
  <Tab title="Build">
    ...
    npx frames render --format webm --output overlay.webm

    # With options
    npx frames render --output output.mp4 --fps 60 --quality high

    # Opt out of local browser GPU capture
    npx frames render --no-browser-gpu --output cpu-browser.mp4

    # Add hardware FFmpeg encoding
    npx frames render --gpu --output gpu.mp4
    ```
    ...
    npx frames render --output default.mp4

    # Override at render time — missing keys fall through to declared defaults
    npx frames render --variables '{"title":"Q4 Report","theme":"dark"}' --output q4.mp4

    # Pass values from a JSON file
    npx frames render --variables-file ./vars.json --output out.mp4
    ```
    ...
    npx frames render --format webm --output captions.webm
  ...
  <Tab title="Utilities">
    ...
    npx frames media-treatment --capabilities --json
    npx frames media-treatment --capability grading --json

    # Analyze and update one selected media element
    npx frames media-treatment \
      --project . \
    ...
    **CI gating.** `frames doctor --json` always exits 0 on successful execution — the command succeeded if it produced valid output. Whether the environment is healthy is carried in the `ok` field of the payload, so a new CLI release (which flips `Version.ok` to `false`) never breaks your pipeline. Pipe through `jq` to gate on the payload instead:
    ...
    ```bash
    npx frames upgrade
    npx frames upgrade --check         # check and exit (no prompt)
    npx frames upgrade --check --json  # machine-readable for agents
    npx frames upgrade --yes           # show upgrade commands without prompting
    ```
    ...
    npx frames browser path

    # Remove cached Chrome download
    npx frames browser clear
    ```

    The `path` subcommand outputs only the path, useful in scripts: `$(npx frames browser path)`.
    ...
    npx frames feedback --rating 10

    # Rating with optional details
    npx frames feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate"

    # Also file a pre-filled GitHub issue with a published minimal repro (opt-in, consented)
    npx frames feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
    ```
    ...
    npx frames skills --claude
    npx frames skills --cursor
    npx frames skills --claude --gemini
    ```
...
2. `FRAMES_API_KEY` environment variable (frames alias)
...
frames auth login --api-key

# Pipe a key from stdin (CI-friendly)
echo "$HEYGEN_API_KEY" | frames auth login --api-key
```
...
```bash
frames auth logout
frames auth logout --keep-api-key   # clear only an OAuth session
frames auth logout --yes            # skip the confirmation prompt
```
...
| `FRAMES_API_KEY` | Alias for `HEYGEN_API_KEY`. |
...
Render a Frames composition on HeyGen's hosted cloud — no local Chrome, no local ffmpeg, no AWS to manage. Sign in once with `frames auth login` and the same credential drives every `cloud` subcommand.
...
- `frames cloud render`: zero-infra option. HeyGen runs the render; you pay per credit. Use when you don't want to manage Chrome/ffmpeg/AWS locally.

### Auth + base URL

`cloud` reuses the credential resolved by `frames auth status`. Override the API base for staging tests with `HEYGEN_API_URL` (default `https://api.heygen.com`).
