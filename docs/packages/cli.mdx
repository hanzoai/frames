...
The `frames` CLI is the primary way to work with Hyperframes. It handles project creation, live preview, rendering, linting, and diagnostics — all from your terminal.
...
<Tabs>
  <Tab title="Agent mode (default)">
    ...
    npx @hanzo/frame init my-video --example blank --video video.mp4
    npx @hanzo/frame render --output output.mp4 --fps 30 --quality standard
    npx @hanzo/frame upgrade --check --json
    ```
  </Tab>
...
```
  ...
  Run: npx @hanzo/frame@latest
```

This is suppressed in CI environments, non-TTY shells, and when `FRAMES_NO_UPDATE_CHECK=1` is set.
...
<Steps>
  <Step title="Create a project">
    ...
    ```bash
    npx @hanzo/frame init --example warm-grain
    ```
    You will be prompted for a project name, or pass it as an argument:
    ```bash
    npx @hanzo/frame init my-video --example warm-grain
    ```
  ...
  <Step title="Render to MP4">
    ...
    ```bash
    npx @hanzo/frame render --output output.mp4
    ```
    Render a specific composition instead of `index.html`:
    ```bash
    npx @hanzo/frame render -c compositions/intro.html -o intro.mp4
    ```
    For deterministic output, add `--docker`:
    ```bash
    npx @hanzo/frame render --docker --output output.mp4
    ```
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Create">
    ...
    npx @hanzo/frame init my-video --example blank --tailwind
    ...
    npx @hanzo/frame add claude-code-window

    # Add a component (effect / snippet)
    npx @hanzo/frame add shader-wipe

    # Target a different project dir
    npx @hanzo/frame add shader-wipe --dir ./my-video

    # Headless / CI (skip clipboard; also: --json for a machine-readable result)
    npx @hanzo/frame add shader-wipe --no-clipboard --json
    ```
    ...
    `add` reads [`frames.json`](#frames-json) at the project root to know which registry to pull from and where to drop files. If the file is missing but the directory looks like a Hyperframes project (has `index.html`), a default `frames.json` is written the first time you run `add`.
    ...
    Trying `add` with an example's name (e.g. `hanzo frame add warm-grain`) emits a clear error pointing you at `init --example`.
    ...
    npx @hanzo/frame transcribe audio.mp3
    npx @hanzo/frame transcribe video.mp4 --model medium.en --language en

    # Import existing transcripts from other tools
    npx @hanzo/frame transcribe subtitles.srt
    npx @hanzo/frame transcribe captions.vtt
    npx @hanzo/frame transcribe openai-response.json

    # Export caption sidecars from transcript.json
    npx @hanzo/frame transcribe transcript.json --to srt
    ```
    ...
    | `--timeout` | Whisper spawn timeout in ms. Overrides the duration+model auto-scaled default. Increase on slow CPUs (e.g. emulated arm64/x64) where medium/large models take many seconds per audio second. Minimum 5000. Env: `FRAMES_TRANSCRIBE_TIMEOUT_MS`. |
    ...
    **Supported transcript formats:**
    ...
    | Format | Source |
    |--------|--------|
    | whisper.cpp JSON | `hanzo frame init --video`, `hanzo frame transcribe` |
...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
    ...
    npx @hanzo/frame tts "Welcome to Frames"

    # Choose a voice
    npx @hanzo/frame tts "Hello world" --voice am_adam

    # Save to a specific file
    npx @hanzo/frame tts "Intro" --voice bf_emma --output narration.wav

    # Adjust speech speed
    npx @hanzo/frame tts "Slow and clear" --speed 0.8

    # Generate Spanish speech (lang auto-detected from the `e` voice prefix)
    npx @hanzo/frame tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav

    # Override the phonemizer (read English text with a French voice)
    npx @hanzo/frame tts "Bonjour le monde" --voice af_heart --lang fr-fr

    # Read text from a file
    npx @hanzo/frame tts script.txt

    # List available voices
    npx @hanzo/frame tts --list
    ```
    ...
    npx @hanzo/frame remove-background avatar.mp4 -o transparent.webm

    # ProRes 4444 .mov for editing round-trip
    npx @hanzo/frame remove-background avatar.mp4 -o transparent.mov

    # Single image → transparent PNG
    npx @hanzo/frame remove-background portrait.jpg -o cutout.png

    # Layer separation: cutout AND inverse-alpha background plate in one pass
    npx @hanzo/frame remove-background avatar.mp4 \
      -o subject.webm --background-output plate.webm

    # Force CPU on a machine that has CoreML or CUDA
    npx @hanzo/frame remove-background avatar.mp4 -o transparent.webm --device cpu

    # Inspect detected providers without rendering
    npx @hanzo/frame remove-background --info
    ```
    ...
    The model is `u2net_human_seg` (MIT, ~168 MB ONNX). Weights download to `~/.cache/frames/background-removal/models/` on first run and are reused thereafter. Peak inference RAM is ~1.5 GB.

    `--device auto` picks CoreML on Apple Silicon, CUDA when available, and CPU otherwise. The CLI bundles the CPU build of `onnxruntime-node`; for CUDA, set `FRAMES_CUDA=1` and provide a GPU-enabled `onnxruntime-node` build.
    ...
    ```bash
    npx @hanzo/frame capture https://stripe.com
    npx @hanzo/frame capture https://linear.app -o linear-capture
    npx @hanzo/frame capture https://example.com --json
    ```
  ...
  <Tab title="Preview">
    ...
    ```bash
    npx @hanzo/frame preview [dir]
    npx @hanzo/frame preview --port 4567
    ```
    ...
    2. **Local studio mode** — if `@frames/studio` is installed in your project's `node_modules`, spawns Vite with full HMR for faster iteration.
    3. **Monorepo mode** — if running from the Hyperframes source repo, spawns the studio dev server directly.

    ### `publish`

    Upload the project and get back a stable `frames.hanzo.ai` URL:

    ```bash
    npx @hanzo/frame publish [dir]
    npx @hanzo/frame publish --yes
    ```
    ...
    `publish` zips the current project, uploads it to the Frames publish backend, and prints a stable `frames.hanzo.ai` URL for that stored project.

    The printed URL already includes the claim token, so opening it on `frames.hanzo.ai` lets the intended user claim the uploaded project and continue editing in the web app.

    This flow does not keep a local preview server alive and does not open a tunnel. The published URL resolves to the persisted project stored in the cloud, so it keeps working after the CLI process exits.
    ...
    ```bash
    npx @hanzo/frame lint [dir]
    npx @hanzo/frame lint [dir] --verbose   # include info-level findings
    npx @hanzo/frame lint [dir] --json      # machine-readable JSON output
    ```
    ```
    ◆  Linting my-project/index.html
    ...
    **Severity levels:**
    ...
    ```bash
    npx @hanzo/frame check [dir]
    npx @hanzo/frame check [dir] --json         # {ok, lint, runtime, layout, motion, contrast, snapshots}
    npx @hanzo/frame check [dir] --snapshots    # annotated overview frames + per-finding crops
    npx @hanzo/frame check [dir] --at 1.5,4,7.25
    npx @hanzo/frame check [dir] --strict       # exit non-zero on warnings too
    ```
    ...
    ```bash
    npx @hanzo/frame beats [dir]
    npx @hanzo/frame beats [dir] --json   # machine-readable JSON output
    ```
    ...
    Requires a local Chrome (the same one used by `render`; run `npx @hanzo/frame browser ensure` if missing). Detection runs the **same** algorithm the Studio uses; results are near-identical (a different headless-Chrome audio sample rate can shift beat times by a frame or two).
    ...
    ```bash
    npx @hanzo/frame inspect [dir]
    npx @hanzo/frame inspect [dir] --json
    npx @hanzo/frame inspect [dir] --samples 15
    npx @hanzo/frame inspect [dir] --at 1.5,4,7.25
    ```
    ...
    ```bash
    npx @hanzo/frame layout [dir] --json
    ```
    ...
    ```bash
    npx @hanzo/frame snapshot my-project --at 2.9,10.4,18.7
    npx @hanzo/frame snapshot my-project --frames 10
    ```
  ...
  <Tab title="Build">
    ...
    npx @hanzo/frame render --format webm --output overlay.webm

    # With options
    npx @hanzo/frame render --output output.mp4 --fps 60 --quality high

    # Opt out of local browser GPU capture
    npx @hanzo/frame render --no-browser-gpu --output cpu-browser.mp4

    # Add hardware FFmpeg encoding
    npx @hanzo/frame render --gpu --output gpu.mp4
    ```
    ...
    | `--variables` | JSON object | — | Variable overrides merged over `data-composition-variables` defaults. Read via `window.__frames.getVariables()` |
    ...
    | `--frames-cache-dir` | path or `off` / `none` / `false` / `0` | `<tmpdir>/frames-extract-cache-<uid>` | Directory for the content-addressed extracted-frame cache. Relocate it off the system drive when the OS temp directory lives on a small partition — long renders can accumulate multi-GB of frames, and on Windows `%TEMP%` defaults to `C:` (reporter `ts=1784219488`, CLI 0.7.58, 15 GB laptop: extract cache exhausted C: mid-render). Pass an opt-out alias (`off`, `none`, `false`, `0`) to disable caching entirely; frames then extract into the render's `workDir` and are cleaned up when the render ends. Env fallback `FRAMES_EXTRACT_CACHE_DIR`. `hanzo frame doctor` reports the effective directory + free space at that location. |
    ...
    <html
      ...
      <body>
        ...
        <script>
          const vars = window.__frames.getVariables();
    ...
    npx @hanzo/frame render --output default.mp4

    # Override at render time — missing keys fall through to declared defaults
    npx @hanzo/frame render --variables '{"title":"Q4 Report","theme":"dark"}' --output q4.mp4

    # Pass values from a JSON file
    npx @hanzo/frame render --variables-file ./vars.json --output out.mp4
    ```
    ...
    npx @hanzo/frame render --format webm --output captions.webm
  ...
  <Tab title="Utilities">
    ...
    npx @hanzo/frame media-treatment --capabilities --json
    npx @hanzo/frame media-treatment --capability grading --json

    # Analyze and update one selected media element
    npx @hanzo/frame media-treatment \
      --project . \
    ...
    **CI gating.** `hanzo frame doctor --json` always exits 0 on successful execution — the command succeeded if it produced valid output. Whether the environment is healthy is carried in the `ok` field of the payload, so a new CLI release (which flips `Version.ok` to `false`) never breaks your pipeline. Pipe through `jq` to gate on the payload instead:
    ...
    ```bash
    npx @hanzo/frame upgrade
    npx @hanzo/frame upgrade --check         # check and exit (no prompt)
    npx @hanzo/frame upgrade --check --json  # machine-readable for agents
    npx @hanzo/frame upgrade --yes           # show upgrade commands without prompting
    ```
    ...
    npx @hanzo/frame browser path

    # Remove cached Chrome download
    npx @hanzo/frame browser clear
    ```

    The `path` subcommand outputs only the path, useful in scripts: `$(npx @hanzo/frame browser path)`.
    ...
    npx @hanzo/frame feedback --rating 10

    # Rating with optional details
    npx @hanzo/frame feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate"

    # Also file a pre-filled GitHub issue with a published minimal repro (opt-in, consented)
    npx @hanzo/frame feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
    ```
    ...
    Install Frames skills for AI coding tools, including first-party runtime adapter skills:
    ...
    npx @hanzo/frame skills --claude
    npx @hanzo/frame skills --cursor
    npx @hanzo/frame skills --claude --gemini
    ```
    ...
    Skills are fetched from GitHub: the `/frames` entry skill (routes "make me a video" to a workflow), the composition contract and Tailwind v4 browser-runtime guidance (`/frames-core`), all animation including the GSAP / Anime.js / CSS / Lottie / Three.js / WAAPI / TypeGPU runtime adapters (`/frames-animation`), creative direction (`/frames-creative`), media preprocessing (`/media-use`), registry block/component wiring (`/frames-registry`), and the video workflows. The `init` command also offers to install skills automatically after scaffolding a project.
    ...
    **Using `hanzo frame skills` is already fine** — as of v0.4.5 the CLI sets `GIT_CLONE_PROTECTION_ACTIVE=0` on the child environment, which is the opt-in knob Git provides for exactly this case. You don't need to do anything.

    **If you ran `npx skills add hanzoai/frames` directly** (bypassing the Frames CLI), set the env var yourself:

    ```bash
    GIT_CLONE_PROTECTION_ACTIVE=0 npx skills add hanzoai/frames
    ```

    This is tracked in [GH #316](https://github.com/hanzoai/frames/issues/316). An upstream fix in the `skills` CLI itself is the right long-term answer; until that lands, the env var is the correct workaround.
  </Tab>
</Tabs>

## hanzo frame auth

Inspect the Hanzo credential the CLI is using. The credential is a Hanzo IAM
token in `HANZO_API_KEY`, read from [Hanzo KMS](https://kms.hanzo.ai) — there is
nothing to sign into here and nothing written to disk.

### Subcommands

#### `auth status`

Show which credential is in use, the identity behind it, and the org balance.
Exits non-zero when nothing is configured or the API rejects the credential, so
scripts can check state.

```bash
hanzo frame auth status
hanzo frame auth status --json   # machine-readable
```
...
| `HANZO_ORG` | The tenant that owns the shared media bucket. |
| `HANZO_BASE_URL` | API base URL (default `https://api.hanzo.ai`). |

The same token covers voice, music, sound effects, images, video, the shared
media catalog, and cloud rendering — see [Authentication](/guides/authentication).

## hanzo frame cloud

Render a Frames composition on Hanzo Cloud — no local Chrome, no local ffmpeg, no AWS to manage. The same `HANZO_API_KEY` drives every `cloud` subcommand.

```bash
hanzo frame auth login                          # one-time
hanzo frame cloud render ./my-video             # zip + upload + poll + download
hanzo frame cloud render ./my-video --no-wait   # submit and exit with the render_id
hanzo frame cloud list                          # browse recent renders
```
...
End-to-end render: zips the project (excluding root `renders`/`snapshots`, `.git`, `node_modules`, `dist`, `.next`, `coverage`, dotfiles, and project `.framesignore` rules), uploads it through the direct-to-S3 asset flow, submits `POST /v3/frames/renders`, polls `GET /v3/frames/renders/{id}` until the render completes or fails, and streams the resulting video to disk.

Use `hanzo frame cloud render --dry-run` to inspect the compressed size and largest included files without authenticating, uploading, or starting a render. Project-specific `.framesignore` rules use gitignore syntax and also apply to `hanzo frame publish`; keep rules narrow because dynamically selected assets may not have obvious static references.

Render parameters mirror the local `hanzo frame render` UX where they overlap:
...
hanzo frame cloud render --quality high --fps 60

# Fire-and-forget with a webhook (no local polling).
hanzo frame cloud render --callback-url https://example.com/hf-hook --no-wait

# Re-render an already-uploaded composition (skips zip + upload).
hanzo frame cloud render --asset-id asst_abc123

# Render from a public URL (no upload).
hanzo frame cloud render --url https://cdn.example.com/site.zip
```
...
```bash
hanzo frame cloud render . --idempotency-key "$(uuidgen)"
```
...
```bash
hanzo frame cloud list
hanzo frame cloud list --limit 50 --json
hanzo frame cloud list --all
```
...
```bash
hanzo frame cloud get hfr_abc123
hanzo frame cloud get hfr_abc123 --json
```
...
```bash
hanzo frame cloud delete hfr_abc123
hanzo frame cloud delete hfr_abc123 --no-confirm --json
```

### When to pick `cloud` vs `lambda` vs local render

- `hanzo frame render` (local): fastest iteration loop. Use during composition authoring.
- `hanzo frame lambda render`: bring-your-own-AWS distributed rendering. Use when you've already invested in AWS and want chunked parallelism on your own account.
- `hanzo frame cloud render`: zero-infra option. Hanzo Cloud runs the render, metered against the org's balance. Use when you don't want to manage Chrome/ffmpeg/AWS locally.

### Auth + base URL

`cloud` reuses the credential resolved by `hanzo frame auth status`. Override the API base for staging tests with `HANZO_BASE_URL` (default `https://api.hanzo.ai`).

## hanzo frame lambda

Deploy Frames distributed rendering to AWS Lambda and drive renders from your laptop or CI.

The `hanzo frame lambda` command group wraps the `@frames/aws-lambda` SDK plus AWS SAM so an end-to-end render is three commands:

```bash
hanzo frame lambda deploy
hanzo frame lambda render ./my-project --width 1920 --height 1080 --wait
hanzo frame lambda destroy   # when you're done
```
...
Builds `packages/aws-lambda/dist/handler.zip` and SAM-deploys the stack at `examples/aws-lambda/template.yaml`. On success, writes `<cwd>/.frames/lambda-stack-<stackName>.json` so the other subcommands don't need to re-derive the bucket / state-machine ARN.
...
```bash
hanzo frame lambda sites create ./my-project
# → siteId: abc1234deadbeef0  (stable across re-runs of the same tree)

hanzo frame lambda render ./my-project --site-id=abc1234deadbeef0 --width 1920 --height 1080
```
...
```bash
hanzo frame lambda render ./my-project \
...
The composition can be parameterised with `--variables` / `--variables-file`, mirroring the local `hanzo frame render` flags. Variables flow into the Step Functions execution input and reach every chunk worker as `window.__hfVariables`. Mismatches against the composition's `data-composition-variables` declaration print as warnings; pass `--strict-variables` to fail the command instead.

```bash
hanzo frame lambda render ./my-template --site-id=abc1234deadbeef0 \
...
```bash
hanzo frame lambda render-batch ./my-template \
  --batch ./users.jsonl \
  --width 1920 --height 1080 \
  --max-concurrent 10
```
...
Pass `--json` for the machine-readable form. Poll each execution via `hanzo frame lambda progress <renderId>` (or use the returned `executionArn`).
...
```bash
hanzo frame lambda render-batch ./my-template --batch ./users.jsonl \
...
```bash
hanzo frame lambda progress hf-render-abcd1234
```
...
hanzo frame lambda policies user

# Print { TrustRelationship, InlinePolicy } for an IAM role (default: cloudformation principal).
hanzo frame lambda policies role --principal=cloudformation

# Validate a checked-in policy still covers the CLI's needs.
hanzo frame lambda policies validate ./infra/iam/frames-deploy.json
```
...
`hanzo frame lambda` keeps per-stack metadata under `<cwd>/.frames/lambda-stack-<name>.json` so the verbs don't need to call `describe-stacks` every time. Commit the file to a repo or `.gitignore` it depending on your workflow — it contains the bucket name, state-machine ARN, and region, none of which are secrets but all of which are AWS-account-identifying.

## hanzo frame cloudrun

The Google Cloud counterpart to `hanzo frame lambda`. Deploys Frames distributed rendering to Cloud Run + Cloud Workflows and drives renders from your laptop or CI. Wraps the `@frames/gcp-cloud-run` SDK plus `terraform` (the module shipped with the package) and `gcloud` / Cloud Build for the image.

```bash
hanzo frame cloudrun deploy --project my-gcp-project
hanzo frame cloudrun render ./my-project --width 1920 --height 1080 --wait
hanzo frame cloudrun destroy --project my-gcp-project   # when you're done
```
...
```bash
hanzo frame cloudrun deploy --project my-gcp-project --region us-central1
hanzo frame cloudrun deploy --project my-gcp-project --image us-central1-docker.pkg.dev/my-gcp-project/frames/frames-render:v1
```

Flags: `--project` (required), `--region` (default `us-central1`), `--image` (skip the build), `--repo` (Artifact Registry repo, default `frames`). Machine sizing / scaling: `--cpu` (1/2/4/8, default 4), `--memory` (e.g. `32Gi`, default `16Gi`), `--max-instances` (render fan-out ceiling, default 100), `--timeout` (per-request seconds, max 3600). Omitted sizing flags keep the module defaults; for anything finer, apply the Terraform module directly.
...
`hanzo frame cloudrun` caches the deployed stack's coordinates under `~/.frames/cloudrun-state.json` (project id, region, bucket, service URL, workflow id) so `render` / `progress` / `destroy` don't need them re-passed. None are secrets, but all are GCP-project-identifying.

## frames.json

`hanzo frame init` writes a `frames.json` file at the root of every new project. `hanzo frame add` reads it to know which registry to pull items from and where to drop them. Edit the file (or delete it to fall back to defaults) to reshape your project layout or point at a custom registry.
...
<CardGroup cols={2}>
  ...
  <Card title="Studio" icon="palette" href="/packages/studio">
    The editor UI that powers `hanzo frame preview`. Use directly to embed in your own app.
  </Card>
