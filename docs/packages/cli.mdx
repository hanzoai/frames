...
<Tabs>
  <Tab title="Agent mode (default)">
    ...
    npx @hanzo/frames init my-video --example blank --video video.mp4
    npx @hanzo/frames render --output output.mp4 --fps 30 --quality standard
    npx @hanzo/frames upgrade --check --json
    ```
  </Tab>
...
<Steps>
  <Step title="Create a project">
    ...
    ```bash
    npx @hanzo/frames init --example warm-grain
    ```
    You will be prompted for a project name, or pass it as an argument:
    ```bash
    npx @hanzo/frames init my-video --example warm-grain
    ```
  ...
  <Step title="Render to MP4">
    ...
    ```bash
    npx @hanzo/frames render --output output.mp4
    ```
    Render a specific composition instead of `index.html`:
    ```bash
    npx @hanzo/frames render -c compositions/intro.html -o intro.mp4
    ```
    For deterministic output, add `--docker`:
    ```bash
    npx @hanzo/frames render --docker --output output.mp4
    ```
  </Step>
</Steps>
...
<Tabs>
  <Tab title="Create">
    ...
    npx @hanzo/frames init my-video --example blank --tailwind
    ...
    npx @hanzo/frames add claude-code-window

    # Add a component (effect / snippet)
    npx @hanzo/frames add shader-wipe

    # Target a different project dir
    npx @hanzo/frames add shader-wipe --dir ./my-video

    # Headless / CI (skip clipboard; also: --json for a machine-readable result)
    npx @hanzo/frames add shader-wipe --no-clipboard --json
    ```
    ...
    npx @hanzo/frames transcribe audio.mp3
    npx @hanzo/frames transcribe video.mp4 --model medium.en --language en

    # Import existing transcripts from other tools
    npx @hanzo/frames transcribe subtitles.srt
    npx @hanzo/frames transcribe captions.vtt
    npx @hanzo/frames transcribe openai-response.json

    # Export caption sidecars from transcript.json
    npx @hanzo/frames transcribe transcript.json --to srt
    ```
...
Word-level transcripts (whisper output) are grouped into readable caption cues on sentence boundaries. Exporting directly from an `.srt`/`.vtt` source keeps its cue boundaries unchanged. When exporting from a `transcript.json` whose entries are already finished cues with no internal spaces (single-word or CJK captions), pass `--preserve-cues` to keep them one cue per entry.
    ...
    npx @hanzo/frames tts "Welcome to Frames"

    # Choose a voice
    npx @hanzo/frames tts "Hello world" --voice am_adam

    # Save to a specific file
    npx @hanzo/frames tts "Intro" --voice bf_emma --output narration.wav

    # Adjust speech speed
    npx @hanzo/frames tts "Slow and clear" --speed 0.8

    # Generate Spanish speech (lang auto-detected from the `e` voice prefix)
    npx @hanzo/frames tts "La reunión empieza a las nueve" --voice ef_dora --output es.wav

    # Override the phonemizer (read English text with a French voice)
    npx @hanzo/frames tts "Bonjour le monde" --voice af_heart --lang fr-fr

    # Read text from a file
    npx @hanzo/frames tts script.txt

    # List available voices
    npx @hanzo/frames tts --list
    ```
    ...
    npx @hanzo/frames remove-background avatar.mp4 -o transparent.webm

    # ProRes 4444 .mov for editing round-trip
    npx @hanzo/frames remove-background avatar.mp4 -o transparent.mov

    # Single image → transparent PNG
    npx @hanzo/frames remove-background portrait.jpg -o cutout.png

    # Layer separation: cutout AND inverse-alpha background plate in one pass
    npx @hanzo/frames remove-background avatar.mp4 \
      -o subject.webm --background-output plate.webm

    # Force CPU on a machine that has CoreML or CUDA
    npx @hanzo/frames remove-background avatar.mp4 -o transparent.webm --device cpu

    # Inspect detected providers without rendering
    npx @hanzo/frames remove-background --info
    ```
    ...
    ```bash
    npx @hanzo/frames capture https://stripe.com
    npx @hanzo/frames capture https://linear.app -o linear-capture
    npx @hanzo/frames capture https://example.com --json
    ```
  ...
  <Tab title="Preview">
    ...
    ```bash
    npx @hanzo/frames publish [dir]
    npx @hanzo/frames publish --yes
    ```
    ...
    ```bash
    npx @hanzo/frames lint [dir]
    npx @hanzo/frames lint [dir] --verbose   # include info-level findings
    npx @hanzo/frames lint [dir] --json      # machine-readable JSON output
    ```
    ```
    ◆  Linting my-project/index.html
    ...
    **Severity levels:**
    ...
    ```bash
    npx @hanzo/frames check [dir]
    npx @hanzo/frames check [dir] --json         # {ok, lint, runtime, layout, motion, contrast, snapshots}
    npx @hanzo/frames check [dir] --snapshots    # annotated overview frames + per-finding crops
    npx @hanzo/frames check [dir] --at 1.5,4,7.25
    npx @hanzo/frames check [dir] --strict       # exit non-zero on warnings too
    ```
    ...
    ```bash
    npx @hanzo/frames beats [dir]
    npx @hanzo/frames beats [dir] --json   # machine-readable JSON output
    ```
    ...
    Requires a local Chrome (the same one used by `render`; run `npx @hanzo/frames browser ensure` if missing). Detection runs the **same** algorithm the Studio uses; results are near-identical (a different headless-Chrome audio sample rate can shift beat times by a frame or two).
    ...
    ```bash
    npx @hanzo/frames inspect [dir]
    npx @hanzo/frames inspect [dir] --json
    npx @hanzo/frames inspect [dir] --samples 15
    npx @hanzo/frames inspect [dir] --at 1.5,4,7.25
    ```
    ...
    ```bash
    npx @hanzo/frames layout [dir] --json
    ```
    ...
    ```bash
    npx @hanzo/frames snapshot my-project --at 2.9,10.4,18.7
    npx @hanzo/frames snapshot my-project --frames 10
    ```
  ...
  <Tab title="Build">
    ...
    npx @hanzo/frames render --format webm --output overlay.webm

    # With options
    npx @hanzo/frames render --output output.mp4 --fps 60 --quality high

    # Opt out of local browser GPU capture
    npx @hanzo/frames render --no-browser-gpu --output cpu-browser.mp4

    # Add hardware FFmpeg encoding
    npx @hanzo/frames render --gpu --output gpu.mp4
    ```
    ...
    npx @hanzo/frames render --output default.mp4

    # Override at render time — missing keys fall through to declared defaults
    npx @hanzo/frames render --variables '{"title":"Q4 Report","theme":"dark"}' --output q4.mp4

    # Pass values from a JSON file
    npx @hanzo/frames render --variables-file ./vars.json --output out.mp4
    ```
    ...
    npx @hanzo/frames render --format webm --output captions.webm
    ...
    ```bash
    npx @hanzo/frames benchmark [dir]
    ```
  ...
  <Tab title="Utilities">
    ...
    npx @hanzo/frames media-treatment --capabilities --json
    npx @hanzo/frames media-treatment --capability grading --json

    # Analyze and update one selected media element
    npx @hanzo/frames media-treatment \
      --project . \
    ...
    **CI gating.** `frames doctor --json` always exits 0 on successful execution — the command succeeded if it produced valid output. Whether the environment is healthy is carried in the `ok` field of the payload, so a new CLI release (which flips `Version.ok` to `false`) never breaks your pipeline. Pipe through `jq` to gate on the payload instead:
    ...
    npx @hanzo/frames upgrade --check         # check and exit (no prompt)
    npx @hanzo/frames upgrade --check --json  # machine-readable for agents
    npx @hanzo/frames upgrade --yes           # show upgrade commands without prompting
    ```
    ...
    npx @hanzo/frames browser path

    # Remove cached Chrome download
    npx @hanzo/frames browser clear
    ```

    The `path` subcommand outputs only the path, useful in scripts: `$(npx @hanzo/frames browser path)`.
    ...
    npx @hanzo/frames feedback --rating 10

    # Rating with optional details
    npx @hanzo/frames feedback --rating 7 --comment "render succeeded but GSAP timeline didn't animate"

    # Also file a pre-filled GitHub issue with a published minimal repro (opt-in, consented)
    npx @hanzo/frames feedback --rating 3 --comment "GSAP timeline froze on seek" --file-issue
    ```
    ...
    ```bash
    npx @hanzo/frames telemetry enable
    npx @hanzo/frames telemetry disable
    npx @hanzo/frames telemetry status
    ```
    ...
    npx @hanzo/frames skills --claude
    npx @hanzo/frames skills --cursor
    npx @hanzo/frames skills --claude --gemini
    ```
