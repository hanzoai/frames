...
- Preview compositions in the browser — use the [CLI](/packages/cli) (`npx @hanzo/frame preview`) or [studio](/packages/studio)
- Render compositions to MP4 — use the [CLI](/packages/cli) (`npx @hanzo/frame render`) or [producer](/packages/producer)
...
The composition linter checks for structural issues that would cause rendering failures or unexpected behavior. You can run it from the CLI with `npx @hanzo/frame lint`, or call it programmatically:
