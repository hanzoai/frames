...
<Tip>
  **Most users do not need to install `@frames/core` directly.** The [CLI](/packages/cli), [producer](/packages/producer), and [studio](/packages/studio) packages all depend on core internally. You only need it if you are doing one of the things listed below.
</Tip>

**Use `@frames/core` when you need to:**
...
- Preview compositions in the browser — use the [CLI](/packages/cli) (`npx @hanzo/frames preview`) or [studio](/packages/studio)
- Render compositions to MP4 — use the [CLI](/packages/cli) (`npx @hanzo/frames render`) or [producer](/packages/producer)
...
| Import | Description |
|--------|-------------|
| `@frames/core` | Types, parsers, generators, adapters, runtime utilities |
| `@frames/core/lint` | Composition linter |
| `@frames/core/compiler` | Timing compiler, HTML compiler, bundler, static guard |
| `@frames/core/runtime` | Pre-built IIFE runtime for browser injection |
...
import { getDefaultStageZoom } from '@frames/core';
```
...
import type { ParsedHtml, CompositionMetadata } from '@frames/core';
...
import { extractCompositionMetadata } from '@frames/core';
...
import { getVariables } from '@frames/core';
const { title } = getVariables<{ title: string }>();

// Validate CLI / host overrides against the declared schema:
import { validateVariables, formatVariableValidationIssue } from '@frames/core';
...
<Info>
  The GSAP + HTML parsing layer now lives in its own standalone package, [`@frames/parsers`](/packages/parsers). Core re-exports the API below for back-compat; import from `@frames/parsers` directly in new code.
</Info>
...
// GSAP parsing, mutation, and constants live in @frames/parsers:
import { parseGsapScript, SUPPORTED_PROPS, SUPPORTED_EASES } from '@frames/parsers/gsap-parser';
import { updateAnimationInScript, addAnimationToScript, removeAnimationFromScript } from '@frames/parsers/gsap-writer-acorn';
import type { GsapAnimation, GsapMethod, ParsedGsap } from '@frames/core';
...
<Info>
  The composition linter now lives in its own package, [`@frames/lint`](/packages/lint) — install it directly to lint a project or HTML string from Node without the CLI. `@frames/core/lint` remains a back-compat re-export.
</Info>

The composition linter checks for structural issues that would cause rendering failures or unexpected behavior. You can run it from the CLI with `npx @hanzo/frames lint`, or call it programmatically:

```typescript
import { lintHyperframeHtml, lintMediaUrls } from '@frames/core/lint';
import type {
  HyperframeLintResult,
  HyperframeLintFinding,
  HyperframeLintSeverity,     // "error" | "warning" | "info"
  HyperframeLinterOptions,
} from '@frames/core/lint';
...
import { compileHtml } from '@frames/core/compiler';
import type { MediaDurationProber } from '@frames/core/compiler';
...
import type { BundleOptions } from '@frames/core/compiler';
...
import { validateHyperframeHtmlContract } from '@frames/core/compiler';
import type {
...
```typescript
import runtime from '@frames/core/runtime';
```
...
import type { MediaVisualStyleProperty } from '@frames/core';
...
<CardGroup cols={2}>
  <Card title="@frames/parsers" icon="code" href="/packages/parsers">
