...
<Warning>
  **Most users should NOT use the engine directly.** Use the [CLI](/packages/cli) (`npx frames render`) or the [producer](/packages/producer) package instead — they handle runtime injection, audio mixing, and encoding for you.
</Warning>
