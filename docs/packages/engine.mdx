...
<Warning>
  **Most users should NOT use the engine directly.** Use the [CLI](/packages/cli) (`npx @hanzo/frames render`) or the [producer](/packages/producer) package instead — they handle runtime injection, audio mixing, and encoding for you.
</Warning>

**Use `@frames/engine` when you need to:**
...
```typescript
import { resolveConfig, DEFAULT_CONFIG } from '@frames/engine';
...
```typescript
import { spawnStreamingEncoder } from '@frames/engine';
...
```typescript
import { parseAudioElements, processCompositionAudio } from '@frames/engine';
...
```typescript
import { createFileServer } from '@frames/engine';
...
import type { HdrTransfer, HdrEncoderColorParams, HdrMasteringMetadata } from '@frames/engine';
