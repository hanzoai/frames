---
title: "@frames/gcp-cloud-run"
description: "Google Cloud Run and Workflows adapter for distributed Frames rendering."
---

The GCP Cloud Run package runs Frames' distributed render primitives on Google Cloud. It ships a Cloud Run HTTP service, a Node client SDK, and a Terraform module for provisioning the bucket, service, workflow, and service accounts.

```bash
npm install @frames/gcp-cloud-run
```

## When to Use

**Use `@frames/gcp-cloud-run` when you need to:**
...
| Import | Description |
|--------|-------------|
| `@frames/gcp-cloud-run` | Server handler, event types, GCS transport, and client SDK exports |
| `@frames/gcp-cloud-run/server` | Cloud Run HTTP service entry point |
| `@frames/gcp-cloud-run/sdk` | Lightweight Node SDK for deploying sites, starting renders, polling progress, and estimating cost |
...
The service is intentionally close to the AWS Lambda adapter: thin cloud transport around the same `@frames/producer/distributed` primitives.
