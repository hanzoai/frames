...
The lint package is the composition linter extracted from core into a dedicated, independently-installable package. It's the **single source of truth** for linting: both the CLI's `frames lint` command and the render-time render-gate consume the same rule engine from here.
...
<Tip>
  This package is the payoff of running validation **as a library** instead of shelling out to the CLI. A Node app (an agent harness, a CI step, an editor plugin) can `import { lintProject } from '@frames/lint'` and lint a composition directory directly — no `npx @hanzo/frames lint` subprocess, no stdout parsing.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx @hanzo/frames lint` wraps this package.
  </Card>
