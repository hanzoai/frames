...
<Tip>
  This package is the payoff of running validation **as a library** instead of shelling out to the CLI. A Node app (an agent harness, a CI step, an editor plugin) can `import { lintProject } from '@frames/lint'` and lint a composition directory directly — no `npx frames lint` subprocess, no stdout parsing.
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx frames lint` wraps this package.
  </Card>
