...
The lint package is the composition linter extracted from core into a dedicated, independently-installable package. It's the **single source of truth** for linting: both the CLI's `hanzo frame lint` command and the render-time render-gate consume the same rule engine from here.

```bash
npm install @frames/lint
```

## When to Use

<Tip>
  This package is the payoff of running validation **as a library** instead of shelling out to the CLI. A Node app (an agent harness, a CI step, an editor plugin) can `import { lintProject } from '@frames/lint'` and lint a composition directory directly — no `npx @hanzo/frame lint` subprocess, no stdout parsing.
</Tip>

**Use `@frames/lint` when you need to:**
...
<Info>
  `@frames/core/lint` still resolves (via a back-compat re-export stub), so existing imports keep working. New code should import from `@frames/lint` directly.
</Info>
...
```typescript
import { lintHyperframeHtml, lintMediaUrls } from '@frames/lint';
...
```typescript
import { lintProject, shouldBlockRender } from '@frames/lint';
import type { ProjectLintResult } from '@frames/lint';
...
The rule engine runs **fully client-side** — no Node.js, no filesystem, no server round-trip. Import from the `@frames/lint/browser` entry to validate composition HTML directly in a browser-only editor or tool:

```typescript
import { lintHyperframeHtml, shouldBlockRender } from '@frames/lint/browser';
...
<Info>
  `lintProject` (which walks a project **directory**) is filesystem-based and is **not** part of the browser entry — import it from the main `@frames/lint` entry in Node.
</Info>
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx @hanzo/frame lint` wraps this package.
  </Card>
