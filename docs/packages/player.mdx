---
title: "@frames/player"
description: "Embeddable web component for playing Frames compositions in any web page."
---

The player package provides a `<frames-player>` custom element that embeds a Frames composition anywhere — in any framework or plain HTML. Zero dependencies, 3KB gzipped.
...
**Use `@frames/player` when you need to:**
...
- Preview during development — use the [CLI](/packages/cli) (`npx @hanzo/frames preview`)
...
```html
<frames-player src="/compositions/intro.html" controls></frames-player>
```
...
```vue
<template>
  <frames-player :src="compositionUrl" controls />
</template>
...
This is the canonical way to bridge the player into editor tools like [`@frames/studio`](/packages/studio). The studio exports a `resolveIframe` helper that handles both direct iframe refs and web-component refs:
...
<Warning>
  **Common gotcha** — if you pass the `<frames-player>` element itself (not `iframeElement`) into a hook or API that expects an `<iframe>`, every `.contentWindow` / `.contentDocument` access returns `null` because the iframe lives inside the player's Shadow DOM. Timeline seek, play, pause, and DOM inspection all silently no-op. **Always extract `iframeElement` first**, or use `resolveIframe` from `@frames/studio` which handles both iframe and web-component hosts transparently.
</Warning>
...
The player communicates with the composition via the Frames runtime bridge protocol (`postMessage`). Existing compositions work without modification.
