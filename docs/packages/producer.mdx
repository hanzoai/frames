...
**Use `@frames/producer` when you need to:**
...
- Render from the command line without writing code — use the [CLI](/packages/cli) (`npx @hanzo/frame render`)
...
```typescript
import { createRenderJob } from '@frames/producer';
...
```typescript
import type { ProgressCallback, RenderStatus } from '@frames/producer';
...
```typescript
import { RenderCancelledError } from '@frames/producer';
...
```typescript
import { startServer } from '@frames/producer/server';
...
```typescript
import { createRenderHandlers, createProducerApp } from '@frames/producer/server';
...
<Info>
  Docker mode requires Docker to be installed and running. Run `npx @hanzo/frame doctor` to verify your environment. See [Deterministic Rendering](/concepts/determinism) for details on what makes Docker mode deterministic.
</Info>
...
```typescript
import { resolveConfig } from '@frames/producer';
