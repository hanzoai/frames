---
title: "@frames/studio-server"
description: "The studio preview/editor backend — a mountable Hono API, extracted from core."
---

The studio-server package is the HTTP backend that powers studio preview and editing — project routes, file serving, preview bundling, thumbnails, and source-mutation helpers. It was extracted from `@frames/core/studio-api` into a dedicated package so an embedder can mount the studio backend **without** depending on core's full surface, and so core no longer ships a web server it doesn't need at render time.

```bash
npm install @frames/studio-server
```

## When to Use

<Tip>
  **Most users do not need this package directly.** The [CLI](/packages/cli) (`npx @hanzo/frames preview`) and [studio](/packages/studio) wire it up for you. Reach for it when you're **embedding** the studio backend into your own server.
</Tip>

**Use `@frames/studio-server` when you need to:**
...
<Info>
  `@frames/core/studio-api` still resolves (via a back-compat re-export stub), so existing imports keep working. New code should import from `@frames/studio-server` directly.
</Info>
...
| Import | Description |
|--------|-------------|
| `@frames/studio-server` | `createStudioApi`, helpers, types |
| `@frames/studio-server/source-mutation` | Source mutation utilities |
| `@frames/studio-server/screenshot-clip` | Element screenshot-clip geometry |
| `@frames/studio-server/manual-edits-render-script` | Manual-edits render body script |
| `@frames/studio-server/studio-motion-render-script` | Studio motion render body script |
| `@frames/studio-server/draft-markers` | Draft gesture-marker attributes |
| `@frames/studio-server/finite-mutation` | Finite-mutation safety checks |
...
```typescript
import { createStudioApi } from '@frames/studio-server';
import type { StudioApiAdapter, ResolvedProject } from '@frames/studio-server';
...
<CardGroup cols={2}>
  ...
  <Card title="CLI" icon="terminal" href="/packages/cli">
    `npx @hanzo/frames preview` wires this server up for you.
  </Card>
</CardGroup>
