...
- Preview compositions during development — use the [CLI](/packages/cli) (`npx @hanzo/frame preview`), which launches the studio for you
...
<Tip>
  **For most development workflows, you do not need to install the studio directly.** Running `npx @hanzo/frame preview` starts the studio automatically with hot reload. Install `@frames/studio` only if you are embedding the editor into your own application.
</Tip>
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/packages/cli">
    Launches the studio via `npx @hanzo/frame preview` — the easiest way to preview compositions.
  </Card>
