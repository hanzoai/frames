...
**Use `@frames/studio` when you need to:**
...
- Preview compositions during development — use the [CLI](/packages/cli) (`npx @hanzo/frames preview`), which launches the studio for you
...
<Tip>
  **For most development workflows, you do not need to install the studio directly.** Running `npx @hanzo/frames preview` starts the studio automatically with hot reload. Install `@frames/studio` only if you are embedding the editor into your own application.
</Tip>
...
bun run --filter @frames/studio dev
```
...
| Import | Description |
|--------|-------------|
| `@frames/studio` | React components, hooks, and types |
| `@frames/studio/tailwind-preset` | Tailwind CSS preset for studio styling |
...
```typescript
import { NLELayout, NLEPreview, CompositionBreadcrumb } from '@frames/studio';
import type { CompositionLevel } from '@frames/studio';
...
import type { TimelineElement } from '@frames/studio';
...
```typescript
import { SourceEditor, PropertyPanel, FileTree } from '@frames/studio';
...
```typescript
import { StudioApp } from '@frames/studio';
...
```typescript
import { useTimelinePlayer } from '@frames/studio';
...
```typescript
import { usePlayerStore, liveTime, formatTime } from '@frames/studio';
...
```typescript
import { useElementPicker } from '@frames/studio';
...
3. **Timeline component** — parses the composition using `@frames/core` to extract clip timing data and renders the visual timeline panel.
...
```typescript
// tailwind.config.ts
import studioPreset from '@frames/studio/tailwind-preset';
...
<CardGroup cols={2}>
  <Card title="CLI" icon="terminal" href="/packages/cli">
    Launches the studio via `npx @hanzo/frames preview` — the easiest way to preview compositions.
  </Card>
