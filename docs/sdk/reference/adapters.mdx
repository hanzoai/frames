...
```typescript
import type { PreviewAdapter } from "@frames/sdk";
```
...
<Note>
  `ElementAtPointResult` and `DraftProps` are the structural shapes a `PreviewAdapter` produces and consumes. They are **not** re-exported from the `@frames/sdk` barrel — you implement against these shapes rather than importing them.
</Note>
...
```typescript
import { createMemoryAdapter } from "@frames/sdk";
...
**Node.js only.** Returns a `PersistAdapter` that reads and writes files under a root directory. Import from the `@frames/sdk/adapters/fs` subpath — this module uses Node `fs/promises` and is excluded from the browser-safe main bundle.
...
<Warning>
  `createFsAdapter` uses Node.js `fs/promises`. Do not import it in browser or edge environments — import from `@frames/sdk/adapters/fs` (the subpath) so bundlers can tree-shake it.
</Warning>
...
```typescript
import { createHeadlessAdapter } from "@frames/sdk";
...
```typescript
import { createIframePreviewAdapter } from "@frames/sdk";
...
```typescript
import { openComposition, createIframePreviewAdapter } from "@frames/sdk";
...
|--------|---------------|
| `PersistAdapter`, `PreviewAdapter`, `PersistVersionEntry` | `@frames/sdk` (types only) |
| `createMemoryAdapter` | `@frames/sdk` |
| `createHeadlessAdapter` | `@frames/sdk` |
| `createIframePreviewAdapter`, `resolveNearestHfElement` | `@frames/sdk` |
| `createFsAdapter`, `FsAdapterOptions` | `@frames/sdk/adapters/fs` |
