---
title: "Types"
description: "All exported types from @frames/sdk."
---

This page documents every type exported from `@frames/sdk`. Types are verified against `packages/sdk/src/types.ts` and the export barrel at `packages/sdk/src/index.ts`.
...
A stable Frames element identifier — the value of the `data-hf-id` attribute stamped on every editable element by `ensureHfIds`. Use `HfId` wherever you would write a raw `string` to signal that the value is an element id.
...
<ResponseField name="children" type="readonly FramesElement[]">
  Direct child elements. Build a flat list with `flatElements()` from `@frames/sdk` utilities.
</ResponseField>
...
```typescript
type ElementSnapshot = FramesElement;
```

Type alias for `FramesElement`. Used in return types of `getElements()` and `getElement()` to signal that the value is a read-only snapshot — it does not update after subsequent mutations.
...
<ResponseField name="roots" type="readonly FramesElement[]">
...
A single keyframe entry for `addWithKeyframes` and `replaceWithKeyframes`. This is a structural shape, not a runtime export — you pass plain objects of this form to those methods; it is **not** re-exported from the `@frames/sdk` barrel, so there is nothing to import.
