...
description: "History module, persist queue, document utilities, constants, and error types exported from @frames/sdk."
---

This page covers everything exported from `@frames/sdk` that is not covered in [`openComposition`](/sdk/reference/open-composition), the [`Composition` interface](/sdk/reference/composition), [edit operations](/sdk/reference/edit-operations), [types](/sdk/reference/types), or [adapters](/sdk/reference/adapters).
...
```typescript
import { createHistory } from "@frames/sdk";
import type { HistoryModule, HistoryOptions, HistoryEntry } from "@frames/sdk";
```
...
```typescript
import { openComposition, createHistory } from "@frames/sdk";
...
```typescript
import { createPersistQueue } from "@frames/sdk";
import type { PersistQueueModule, PersistQueueOptions } from "@frames/sdk";
```
...
```typescript
import { openComposition, createPersistQueue } from "@frames/sdk";
...
```typescript
import { buildDocument, buildRoots, flatElements } from "@frames/sdk";
```

Low-level utilities for building the `SdkDocument` model and `FramesElement` trees from parsed HTML. These are the same functions the SDK uses internally on every `openComposition()` call. You rarely need them directly — they are exposed for hosts that parse HTML outside of a session, or for testing.
...
```typescript
function buildRoots(document: Document): FramesElement[];
```
...
```typescript
function flatElements(roots: readonly FramesElement[]): FramesElement[];
```
...
```typescript
import { buildDocument, flatElements } from "@frames/sdk";
...
```typescript
import { resolveScoped, findById, bareId, escapeHfId, isNewHostBoundary } from "@frames/sdk";
```
...
```typescript
import { readVariableDefault } from "@frames/sdk";
```
...
```typescript
import { ORIGIN_APPLY_PATCHES } from "@frames/sdk";

const ORIGIN_APPLY_PATCHES = "@frames/sdk:applyPatches";
```
...
The value is a namespaced string rather than a `Symbol` so it survives realm boundaries — `postMessage`, structured clone, and JSON serialization all preserve it correctly. T3 embedded hosts that forward patch events across frames or workers rely on this property. The namespace prefix (`@frames/sdk:`) makes accidental collision with a host-chosen origin string negligible.

### ORIGIN\_LOCAL

```typescript
import { ORIGIN_LOCAL } from "@frames/sdk";
...
```typescript
import { UnsupportedOpError } from "@frames/sdk";
```
...
```typescript
import { resolveNearestHfElement } from "@frames/sdk";
...
```typescript
import { resolveElementAffordances } from "@frames/sdk/editing";
```

Determines which editing operations are available for a live element given its current DOM state and model. Imported from the `@frames/sdk/editing` subpath. Full documentation is in the [Editing Affordances guide](/sdk/guides/editing-affordances).
