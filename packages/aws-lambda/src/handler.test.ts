 ...
 * `@hanzo/frames-producer/distributed` and have their own coverage in
...
describe("handler dispatch", () => {
  it("routes Action='plan' to the plan primitive", async () => {
    ...
    const result = await handler(event, {
      ...
      primitives: {
        plan: planMock as unknown as typeof import("@hanzo/frames-producer/distributed").plan,
        renderChunk:
          renderChunkMock as unknown as typeof import("@hanzo/frames-producer/distributed").renderChunk,
        assemble:
          assembleMock as unknown as typeof import("@hanzo/frames-producer/distributed").assemble,
      },
      tmpRoot,
        ...
        handler(
          ...
          },
          {
            ...
            primitives: {
              plan: mock(async () => {
                throw terminal;
              }) as unknown as typeof import("@hanzo/frames-producer/distributed").plan,
              renderChunk: mock(async () => {
                throw new Error("unused");
              }) as unknown as typeof import("@hanzo/frames-producer/distributed").renderChunk,
              assemble: mock(async () => {
                throw new Error("unused");
              }) as unknown as typeof import("@hanzo/frames-producer/distributed").assemble,
            },
            tmpRoot,
            skipChromeResolution: true,
          },
  ...
  it("runs v2 plan → target-scoped chunk → assemble without a PlanS3Uri", async () => {
    ...
    const deps = {
      ...
      primitives: {
        ...
        planV2WithPublisher:
          planV2WithPublisherMock as unknown as typeof import("@hanzo/frames-producer/distributed").planV2WithPublisher,
        renderChunk:
          renderChunkMock as unknown as typeof import("@hanzo/frames-producer/distributed").renderChunk,
        assemble:
          assembleMock as unknown as typeof import("@hanzo/frames-producer/distributed").assemble,
...
describe("handler — S3 URI allowlist (security: F-004)", () => {
  ...
  beforeEach(() => {
    prevBucket = process.env.FRAMES_RENDER_BUCKET;
  });

  afterEach(() => {
    if (prevBucket === undefined) {
      delete process.env.FRAMES_RENDER_BUCKET;
    } else {
      process.env.FRAMES_RENDER_BUCKET = prevBucket;
    }
  });

  it("rejects a plan event whose ProjectS3Uri is outside the allowed bucket", async () => {
    process.env.FRAMES_RENDER_BUCKET = "good-bucket";
    const tmpRoot = makeTmpRoot();
    const s3 = new FakeS3Client();
