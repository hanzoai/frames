...
// every HeyGen call (both auth types) so the backend can isolate frames CLI
...
export const HEYGEN_CLIENT_SOURCE = "frames";
...
export function buildAuthHeaders(credential: ResolvedCredential): Record<string, string> {
  ...
  // sent here — an API-key frames call is still frames usage.
