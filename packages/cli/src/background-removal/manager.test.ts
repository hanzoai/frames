...
describe("background-removal/manager — selectProviders", () => {
  beforeEach(() => {
    vi.resetModules();
    delete process.env["FRAMES_CUDA"];
  });
  ...
  it("auto falls back to CPU on linux without FRAMES_CUDA", async () => {
    vi.doMock("node:os", () => ({
      platform: () => "linux",
      arch: () => "x64",
      homedir: () => "/tmp",
  ...
  it("auto picks CUDA on linux when FRAMES_CUDA=1", async () => {
    process.env["FRAMES_CUDA"] = "1";
    vi.doMock("node:os", () => ({
      platform: () => "linux",
      arch: () => "x64",
      homedir: () => "/tmp",
