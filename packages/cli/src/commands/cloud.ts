/**
 * `frames cloud` — top-level dispatcher for cloud-render subverbs.
 *
...
export const examples: Example[] = [
  ...
  ["Render a specific project", "frames cloud render ./my-video"],
  ["Inspect the project zip without uploading", "frames cloud render --dry-run"],
  [
    "Render at 60fps + high quality, save to a path",
    "frames cloud render ./my-video --fps 60 --quality high -o ./out.mp4",
  ],
  [
    "Fire-and-forget with a webhook",
    "frames cloud render ./my-video --callback-url https://example.com/hf-hook --no-wait",
  ],
  ["Resubmit an already-uploaded zip", "frames cloud render --asset-id asst_abc123"],
  [
    "Render from a public HTTPS zip",
    "frames cloud render --url https://cdn.example.com/site.zip",
  ],
  ["List recent cloud renders", "frames cloud list"],
  ["Fetch one render's status + signed URLs", "frames cloud get hfr_abc123"],
  ["Soft-delete a render", "frames cloud delete hfr_abc123"],
];

const HELP = `
${c.bold("frames cloud")} ${c.dim("<subcommand> [args]")}

Render Frames compositions on HeyGen's cloud infrastructure. The
...
${c.bold("AUTH:")}
  Uses the credential you signed in with via ${c.accent("frames auth login")}.
  Override the API base with ${c.accent("HEYGEN_API_URL")} (default https://api.heygen.com).
`;

export default defineCommand({
  meta: { name: "cloud", description: "Render Frames compositions on the HeyGen cloud" },
  subCommands: {
