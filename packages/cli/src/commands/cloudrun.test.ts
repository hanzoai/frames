...
describe("Cloud Run adapter preflight", () => {
  it("identifies only the missing adapter, not a missing transitive dependency", () => {
    const adapterError = Object.assign(
      new Error("Cannot find package '@hanzo/frames-gcp-cloud-run' imported from cli.js"),
  ...
  it("provides global and project-local install recovery commands", () => {
    ...
    expect(message).toContain("npm install -g @hanzo/frames-gcp-cloud-run");
    expect(message).toContain("npm install @hanzo/frames-gcp-cloud-run");
  });
});
