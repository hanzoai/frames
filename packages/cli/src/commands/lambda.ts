 ...
 * lives in `@hanzo/frames-aws-lambda/sdk`.
 */

import { defineCommand } from "citty";
import type { DistributedFormat } from "@hanzo/frames-aws-lambda/sdk";
...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    // The lambda subverbs dynamic-import `@hanzo/frames-aws-lambda` at call
    ...
    if (verbsNeedingSDK.has(subcommand)) {
      try {
        await import("@hanzo/frames-aws-lambda/sdk");
      } catch (err) {
        if ((err as NodeJS.ErrnoException).code === "ERR_MODULE_NOT_FOUND") {
          console.error(
            `${c.error("@hanzo/frames-aws-lambda is not installed.")} The ${c.accent(`frames lambda ${subcommand}`)} command needs it at runtime.\n` +
              `Install it alongside the CLI:\n` +
              `  ${c.accent("npm install -g @hanzo/frames-aws-lambda")}\n` +
              `Or, for an opt-in dev setup:\n` +
              `  ${c.accent("npm install @hanzo/frames-aws-lambda")}`,
          );
          failCommand();
        }
        throw err;
