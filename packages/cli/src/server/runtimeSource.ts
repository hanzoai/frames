 ...
 *   2. Inlined constant    (production — baked into @hanzo/frames-core at build time)
...
async function buildFromSource(): Promise<string | null> {
  ...
  try {
    const mod = await import("@hanzo/frames-core");
...
function readFromNodeModules(): string | null {
  const subPaths = ["node_modules/@hanzo/frames/dist", "node_modules/@hanzo/frames-core/dist"];
  let dir = __dirname;
  for (;;) {
