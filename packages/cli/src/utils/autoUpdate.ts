 ...
 *
 * Flow across two runs of `frames`:
 *
 ...
 *     or when `FRAMES_NO_AUTO_INSTALL` / `FRAMES_NO_UPDATE_CHECK`
 *     is set.
 *   - If a previous install is still in flight (less than 10 min old), don't
 *     re-launch.
 *   - Installer output is redirected to `~/.frames/auto-update.log` for
...
function isAutoInstallDisabled(): boolean {
  ...
  if (process.env["FRAMES_NO_UPDATE_CHECK"] === "1") return true;
  if (process.env["FRAMES_NO_AUTO_INSTALL"] === "1") return true;
  return false;
}
...
function launchDetachedInstall(
  ...
  const child = spawn(process.execPath, ["-e", nodeScript], {
    ...
    windowsHide: true,
    env: { ...process.env, FRAMES_NO_UPDATE_CHECK: "1", FRAMES_NO_AUTO_INSTALL: "1" },
  });
  child.unref();
...
export function reportCompletedUpdate(): void {
  if (process.env["FRAMES_NO_UPDATE_CHECK"] === "1") return;
  ...
  if (done.ok) {
    process.stderr.write(`  frames auto-updated to v${done.version}\n\n`);
  } else if (!done.reported) {
    // Failed installs are surfaced once too — the user should know why the
    // auto-update didn't take.
    process.stderr.write(
      `  frames auto-update to v${done.version} failed. Run \`frames upgrade\` to retry.\n\n`,
    );
  }
}
