...
describe("detectInstaller", () => {
  ...
  it("classifies workspace link as skip (monorepo dev)", async () => {
    const info = await detectWith("/Users/dev/frames-oss/packages/cli/dist/cli.js");
  ...
  it("classifies bunx temp dir as skip", async () => {
    const info = await detectWith("/var/folders/tmp/bunx-501-frames/entry.js");
    expect(info.kind).toBe("skip");
    expect(info.reason.toLowerCase()).toContain("ephemeral");
  });

  it("detects Homebrew install", async () => {
    const info = await detectWith("/opt/homebrew/Cellar/frames/0.4.3/bin/frames");
    expect(info.kind).toBe("brew");
    expect(info.installCommand("0.4.4")).toBe("brew upgrade frames");
  });
  ...
  it("returns skip for an unknown install layout", async () => {
    const info = await detectWith("/some/random/path/frames");
    expect(info.kind).toBe("skip");
