...
describe("FRAMES_PIN_RE", () => {
  it("matches a frames@<semver> token and captures the version", () => {
    const match = "npx --yes @hanzo/frames@1.2.3 render".match(FRAMES_PIN_RE);
...
describe("rewriteProjectPinnedScripts", () => {
  const scripts = {
    dev: "npx --yes @hanzo/frames@0.7.48 preview",
    check: "npx --yes @hanzo/frames@0.7.48 check",
    render: "npx --yes @hanzo/frames@0.7.48 render",
    unrelated: "echo hi",
  ...
  it("bumps every pinned frames script to the target, leaving others untouched", () => {
    ...
    expect(r.scripts.render).toBe("npx --yes @hanzo/frames@0.7.55 render");
    expect(r.scripts.dev).toBe("npx --yes @hanzo/frames@0.7.55 preview");
  ...
  it("is a no-op when already at target", () => {
    const at = rewriteProjectPinnedScripts({ render: "npx --yes @hanzo/frames@0.7.55 render" }, "0.7.55");
  ...
  it("reads distinct pinned versions across scripts", () => {
    expect(
      readPinnedHyperframesVersions({
        a: "npx --yes @hanzo/frames@0.7.48 render",
