...
describeE2E("publish stable-URL round trip (live server)", () => {
  ...
  it("two identities in a shared team space converge on one URL, and other spaces can't hijack it", async () => {
    ...
    try {
      ...
      process.env["FRAMES_API_KEY"] = "e2e-team-member-b";
      ...
      process.env["FRAMES_API_KEY"] = "e2e-outsider";
      ...
      if (originalKey === undefined) delete process.env["FRAMES_API_KEY"];
      else process.env["FRAMES_API_KEY"] = originalKey;
      rmSync(dir, { recursive: true, force: true });
    }
  }, 120_000);
});
