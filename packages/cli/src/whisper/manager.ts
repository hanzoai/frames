...
const MODELS_DIR = join(homedir(), ".cache", "frames", "whisper", "models");
...
function findFromEnv(): WhisperResult | undefined {
  const envPath = process.env["FRAMES_WHISPER_PATH"];
...
const BUILD_DIR = join(homedir(), ".cache", "frames", "whisper", "whisper.cpp");
...
function buildFromSource(onProgress?: (msg: string) => void): WhisperResult {
  ...
  if (!existsSync(BUILD_DIR)) {
    ...
    mkdirSync(join(homedir(), ".cache", "frames", "whisper"), {
      recursive: true,
    });
