...
describe("swallow", () => {
  ...
  it("logs to console.debug when window.__hfDebug is true", () => {
    ...
    expect(console.debug).toHaveBeenCalledWith("[frames] test.debug swallowed:", err);
  });

  it("also honors window.__FRAMES_DEBUG (legacy flag)", () => {
    w.__FRAMES_DEBUG = true;
    swallow("test.legacy", "string-error");
    expect(console.debug).toHaveBeenCalledWith(
      "[frames] test.legacy swallowed:",
      "string-error",
    );
  });
