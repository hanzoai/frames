 ...
 * It dispatches to:
 *
 *   - `console.debug` with the label, the error, and a `[frames]` prefix
 *     when `window.__hfDebug === true` (or the legacy `__FRAMES_DEBUG`
...
export function swallow(label: string, error?: unknown): void {
  ...
  if (w.__hfDebug || w.__FRAMES_DEBUG) {
    // eslint-disable-next-line no-console -- intentional debug surface
    console.debug(`[frames] ${label} swallowed:`, error);
  }
}
