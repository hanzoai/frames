...
the GCP counterpart to [`@hanzo/frames-aws-lambda`](../aws-lambda).
...
The package is **not** a dependency of `@hanzo/frames-producer`; install it
separately.
...
# 2. Apply the module.
terraform -chdir=node_modules/@hanzo/frames-gcp-cloud-run/terraform init
terraform -chdir=node_modules/@hanzo/frames-gcp-cloud-run/terraform apply \
...
```ts
import { renderToCloudRun, getRenderProgress } from "@hanzo/frames-gcp-cloud-run/sdk";
