...
```bash
npm install @hanzo/frames-player
```

Or load directly via CDN:

```html
<script type="module" src="https://cdn.jsdelivr.net/npm/@hanzo/frames-player"></script>
```

If you need a classic `<script>` tag instead of ESM, use the explicit global build:

```html
<script src="https://cdn.jsdelivr.net/npm/@hanzo/frames-player/dist/frames-player.global.js"></script>
```

## Usage

```html
<frames-player src="./my-composition/index.html" controls></frames-player>
```
...
When a composition uses `@hanzo/frames-shader-transitions`, the player can own preview-only shader capture settings:
...
```js
const player = document.querySelector("frames-player");
...
This is the canonical way to bridge the player into tools like [`@hanzo/frames-studio`](../studio). The studio exports a `resolveIframe` helper that works with both iframe refs and web-component refs:

```ts
import { useTimelinePlayer, resolveIframe } from "@hanzo/frames-studio";
...
```tsx
import "@hanzo/frames-player";
import type { HyperframesPlayer } from "@hanzo/frames-player";
import { useTimelinePlayer, resolveIframe } from "@hanzo/frames-studio";
...
The player renders compositions in a sandboxed `<iframe>` inside a Shadow DOM. It communicates with the Frames runtime via `postMessage`. If the composition has GSAP timelines (`window.__timelines`) but no runtime, the player auto-injects it from CDN.
