...
```bash
npm install @hanzo/frames-producer
```
...
```typescript
import { createRenderJob, executeRenderJob } from "@hanzo/frames-producer";
...
```typescript
import { startServer } from "@hanzo/frames-producer";
...
```typescript
import { plan, renderChunk, assemble } from "@hanzo/frames-producer/distributed";
...
Full documentation: [frames.hanzo.ai/packages/producer](https://frames.hanzo.ai/packages/producer)

## Related packages

- [`@hanzo/frames-core`](../core) — types, parsers, frame adapters
- [`@hanzo/frames-engine`](../engine) — lower-level capture and encode primitives
- [`frames`](../cli) — CLI
