...
import { type CanvasResolution, fpsToNumber } from "@hanzo/frames-core";
import {
  type EngineConfig,
...
export interface DistributedRenderConfig {
   ...
   * `@hanzo/frames-aws-lambda` SDK) also validate the shape client-side
...
/**
 * Hash the deterministic-font bundle that ships inside `@hanzo/frames-producer`.
...
export async function plan(
  ...
  if (probeResult.probeSession) {
    ...
    try {
      const { closeCaptureSession } = await import("@hanzo/frames-engine");
  ...
  // approach left a duplicate under `compiled/__frames_video_frames/`)
  // halves peak disk usage during `plan()`.
  const stagedVideoFrames = join(compiledDir, "__frames_video_frames");
