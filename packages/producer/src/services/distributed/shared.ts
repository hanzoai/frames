...
import { type VideoElement, type VideoFrameFormat, type VideoMetadata } from "@frames/engine";
 ...
 * module until a `package.json` whose `name === "@frames/producer"`
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (existsSync(candidate)) {
      try {
        ...
        if (pkg.name === "@frames/producer" && typeof pkg.version === "string") {
