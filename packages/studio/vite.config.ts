...
function devProjectApi(): Plugin {
  ...
  return {
    ...
    configureServer(server): void {
      ...
      const getApi = async () => {
        if (!_api) {
          const mod = await server.ssrLoadModule("@frames/studio-server");
...
export default defineConfig({
  ...
  resolve: {
    alias: {
      "@frames/player": resolve(__dirname, "../player/src/frames-player.ts"),
      "@frames/studio-server/source-mutation": resolve(
        __dirname,
  ...
  ssr: {
    ...
    // reachable only server-side via the Node-only `@frames/parsers/gsap-parser`
