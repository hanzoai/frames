 ...
 * deterministic on ANY machine: frames' renderer only auto-supplies its
 * ~18 CANONICAL_FONTS; every other family falls back to a generic font unless
 * the HTML ships a local @font-face. (frames' own font linter says exactly
...
const header =
  ...
  `   These are the template fonts NOT in frames' auto-resolved set; inlining\n` +
