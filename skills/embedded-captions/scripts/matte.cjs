#!/usr/bin/env node
/*
 * matte.cjs — subject matting via frames' built-in `remove-background`
 ...
 * weights — the model auto-downloads once (~168 MB) to ~/.cache/frames/.
 *
 ...
 *
 * Pipeline:
 *   source.mp4 → frames remove-background → ProRes 4444 .mov (lossless
 ...
 * Env:    FRAMES_ROOT — frames checkout (default ~/Downloads/frames)
 */
...
function hfCli() {
  ...
  console.error("[matte] cannot find frames cli — set FRAMES_ROOT to a built checkout");
  process.exit(3);
}
...
async function main() {
  ...
  // 1) subject matte via frames (ProRes 4444 keeps the alpha lossless).
  ...
  console.log(
    `[matte] frames remove-background (u2net_human_seg${cached ? "" : "; first run downloads ~168 MB"})… model load takes ~1-2 min with no output — not hung`,
  );
