...
// Locate frames' bundled puppeteer. render-and-composite.sh exports
// FRAMES_ROOT; standalone we also try the in-repo path + ~/Downloads, and
...
if (!puppeteer) {
  console.error(
    "[measure] could not locate puppeteer — set FRAMES_ROOT to a built frames checkout",
  );
  process.exit(3);
}

// Resolve frames' bundled GSAP. The templates load GSAP from a CDN
