#!/usr/bin/env bash
# End-to-end: frames render + ffmpeg overlay matte → final.mp4
#
# Usage:
#   bash render-and-composite.sh <project-dir> [frames-repo-path]
#
# Env:
#   FRAMES_ROOT  override the frames checkout location

set -euo pipefail

PROJECT="${1:?usage: render-and-composite.sh <project-dir> [frames-repo]}"
PROJECT="$(cd "$PROJECT" && pwd)"

# Resolve the frames checkout. Candidate order:
#   1. arg 2   2. $FRAMES_ROOT   3. repo root if this skill ships INSIDE the
#   frames repo (skills/embedded-captions/scripts → ../../..)   4. ~/Downloads/frames
SKILL_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
HF=""
for cand in "${2:-}" "${FRAMES_ROOT:-}" "$(cd "$SKILL_SCRIPT_DIR/../../.." 2>/dev/null && pwd)" "$HOME/Downloads/frames"; do
  if [[ -n "$cand" && -f "$cand/packages/cli/dist/cli.js" ]]; then HF="$cand"; break; fi
done
if [[ -z "$HF" ]]; then
  echo "[render] frames CLI not found. Set FRAMES_ROOT to your frames" >&2
  echo "         checkout (needs packages/cli/dist/cli.js — 'bun install && bun run build')." >&2
  exit 1
fi
export FRAMES_ROOT="$HF"   # so the occlusion gate's measure-layout.cjs finds puppeteer too
...
# Embed template fonts BEFORE the gates + render. frames only auto-supplies
...
echo "[render] frames render @ ${FPS}fps"
...
# hf_render_dir: render one frames composition.
...
# the ORIGINAL video filename `hanzo frame init` scaffolds (e.g. clip.mp4), not just a
...
# Hybrid renders need 2 independent frames passes. They share no state, so
...
if [[ -f "$PROJECT/index_fg.html" ]]; then
  ...
  # Standard mode: TWO independent frames passes (base = index.html with the
