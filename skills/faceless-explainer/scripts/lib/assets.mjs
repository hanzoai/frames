...
export function stageAssets({ framesDir, frames }) {
  const wanted = new Set();
  for (const f of frames) {
    for (const b of basenamesFromCandidates(f.extra?.asset_candidates)) wanted.add(b);
  }
  const captureDirs = [
    join(framesDir, "capture/assets"),
    join(framesDir, "capture/assets/videos"), // videos download into a subdir
    join(framesDir, "capture/screenshots"),
  ];
  const assetsDir = join(framesDir, "assets");
  const anomalies = [];
  let staged = 0;
  if (wanted.size > 0) {
