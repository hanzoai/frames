...
Run commands as `npx @hanzo/frames ...` unless project instructions provide a wrapper. Obey the wrapper when present. The CLI requires Node.js 22 or newer and FFmpeg.

## Development loop

1. **Scaffold:** `npx @hanzo/frames init <project>` or capture a site. In non-TTY mode, pass `--non-interactive --example=<name>`.
2. **Author:** write the composition using `/frames-core`.
3. **Get fast feedback while editing:** run `npx @hanzo/frames lint` after the first HTML pass and after structural changes.
4. **Run the final gate:** run `npx @hanzo/frames check`; it reruns lint before opening the browser. Do not prepend a redundant standalone lint invocation. Add `--snapshots` for annotated overview frames and finding crops.
...
6. **Open the final Studio preview:** run `npx @hanzo/frames preview`, hand the timeline project URL to the user, and ask whether to revise or render.
...
npx @hanzo/frames render --quality high --output out.mp4
test -s out.mp4
...
```bash
npx @hanzo/frames snapshot --at <t1>,<t2>,<t3>
```
...
- `doctor --json` always exits zero. Gate on its payload:
  ...
  ```bash
  npx @hanzo/frames doctor --json | jq -e '.ok' >/dev/null
  ```
...
| Need                                | Command                                                                         |
| ----------------------------------- | ------------------------------------------------------------------------------- |
| Fast local iteration                | `npx @hanzo/frames render --quality draft`                                      |
| Final local delivery                | `npx @hanzo/frames render --quality high --output out.mp4`                      |
| Reproducible container render       | `npx @hanzo/frames render --docker --strict --output out.mp4`                   |
| Local variable-driven batch render  | `npx @hanzo/frames render --batch rows.json --output "renders/{name}.mp4"`      |
| Hosted zero-infrastructure render   | `npx @hanzo/frames cloud render`                                                |
| Self-managed distributed AWS render | `npx @hanzo/frames lambda render <project> --width 1920 --height 1080 --wait`   |
| Self-managed distributed GCP render | `npx @hanzo/frames cloudrun render <project> --width 1920 --height 1080 --wait` |
...
```bash
npx @hanzo/frames feedback --rating <0-10> --comment "<specific result or friction>"
```
...
```bash
npx @hanzo/frames present <project-dir> --port 3004 --no-open
npx @hanzo/frames beats <project-dir> --json
npx @hanzo/frames keyframes <project-dir> --json
```
