...
```bash
npx @hanzo/frames info                   # project metadata
npx @hanzo/frames info ./my-video        # specific project
npx @hanzo/frames info --json
```
...
```bash
npx @hanzo/frames upgrade                # check + interactive prompt
npx @hanzo/frames upgrade --check        # check and exit, no prompt (agent-friendly)
npx @hanzo/frames upgrade --check --json # machine-readable: current / latest / updateAvailable
npx @hanzo/frames upgrade --yes          # print upgrade commands without prompting
```

Compares the installed CLI version against npm latest.

`--project [dir]` bumps a **project's** pinned scripts instead of the global install: it rewrites every `npx …frames@<version>…` in `<dir>/package.json` (default cwd) to npm-latest. Always invoke it unpinned (`npx @hanzo/frames@latest upgrade --project`) — a project scaffolded on an old CLI stays frozen otherwise. `--project . --check` reports the delta without writing; add `--json` for `{ changed, from, to, path }`. Pass the dir explicitly whenever another flag follows `--project` — on older releases a bare `--project` consumes the next flag as its directory value.

## compositions, docs

```bash
npx @hanzo/frames compositions           # list compositions in project
npx @hanzo/frames compositions --json
npx @hanzo/frames docs                   # list available topics
npx @hanzo/frames docs rendering         # print one topic inline in the terminal
```
...
```bash
npx @hanzo/frames benchmark              # run the preset matrix in current project
npx @hanzo/frames benchmark ./my-video   # specific project
npx @hanzo/frames benchmark --runs 5     # repeat each config N times (default 3)
npx @hanzo/frames benchmark --json
```
...
```bash
npx @hanzo/frames telemetry status      # show telemetry state
npx @hanzo/frames telemetry disable     # disable anonymous usage telemetry
npx @hanzo/frames telemetry enable      # re-enable telemetry
```
...
```bash
npx @hanzo/frames tts
npx @hanzo/frames transcribe
npx @hanzo/frames remove-background
```
