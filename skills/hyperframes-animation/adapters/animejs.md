---
name: frames-animejs
description: Anime.js adapter patterns for Frames. Use when writing Anime.js animations or timelines inside Frames compositions, registering animations on window.__hfAnime, making Anime.js seek-driven and deterministic, or translating Anime.js examples into render-safe Frames HTML.
---

# Anime.js for Frames

Frames can seek Anime.js instances through its `animejs` runtime adapter. The composition owns the animation objects; Frames owns the clock.
...
The adapter seeks every registered instance with `instance.seek(timeMs)`, where `timeMs` is Frames time in milliseconds.
...
Use GSAP for complex scene sequencing unless the user specifically asks for Anime.js. GSAP is still the primary Frames authoring path.
...
- Frames adapter source: `packages/core/src/runtime/adapters/animejs.ts`.
