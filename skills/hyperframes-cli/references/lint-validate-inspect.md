...
```bash
npx frames lint                  # current directory
npx frames lint ./my-project     # specific project
npx frames lint --verbose        # info-level findings
npx frames lint --json           # machine-readable
```
...
```bash
npx frames check                    # current directory: the full browser gate
npx frames check ./my-project       # specific project
npx frames check --json             # agent-readable envelope {ok, lint, runtime, layout, motion, contrast, snapshots}
npx frames check --snapshots        # also write overview frames (annotated) + per-finding crops
npx frames check --samples 15       # denser timeline sweep (default 9)
npx frames check --at 1.5,4,7.25    # explicit hero-frame timestamps
npx frames check --at-transitions   # also sample every tween start/end boundary
npx frames check --tolerance 4      # allowed overflow px before reporting (default 2)
npx frames check --timeout 5000     # ms for the initial settle (default 3000)
npx frames check --no-contrast      # skip the WCAG audit while iterating
npx frames check --strict           # exit non-zero on warnings too (default: only errors)
```
...
```bash
npx frames check --caption-zone "x0=0;y0=.82;x1=1;y1=1;severity=error;seek=.25,1"
npx frames check --frame-check     # media (img/svg/video/canvas) out-of-frame detection
```
...
```bash
npx frames snapshot                       # 5 key frames as PNG
npx frames snapshot ./my-project          # specific project
npx frames snapshot --frames 10           # evenly-spaced N frames
```
...
```bash
npx frames check --snapshots               # reports a finding, e.g. content_overlap on "#cta"
npx frames snapshot --zoom "#cta"           # crop the element to verify the defect, at 3x density
npx frames snapshot --zoom "100,50,400,300" --zoom-scale 2   # or an exact pixel region
