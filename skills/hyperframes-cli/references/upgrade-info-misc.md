...
```bash
npx @hanzo/frame info                   # project metadata
npx @hanzo/frame info ./my-video        # specific project
npx @hanzo/frame info --json
```
...
```bash
npx @hanzo/frame upgrade                # check + interactive prompt
npx @hanzo/frame upgrade --check        # check and exit, no prompt (agent-friendly)
npx @hanzo/frame upgrade --check --json # machine-readable: current / latest / updateAvailable
npx @hanzo/frame upgrade --yes          # print upgrade commands without prompting
```

Compares the installed CLI version against npm latest.

`--project [dir]` bumps a **project's** pinned scripts instead of the global install: it rewrites every `npx …frames@<version>…` in `<dir>/package.json` (default cwd) to npm-latest. Always invoke it unpinned (`npx @hanzo/frame@latest upgrade --project`) — a project scaffolded on an old CLI stays frozen otherwise. `--project . --check` reports the delta without writing; add `--json` for `{ changed, from, to, path }`. Pass the dir explicitly whenever another flag follows `--project` — on older releases a bare `--project` consumes the next flag as its directory value.

## compositions, docs

```bash
npx @hanzo/frame compositions           # list compositions in project
npx @hanzo/frame compositions --json
npx @hanzo/frame docs                   # list available topics
npx @hanzo/frame docs rendering         # print one topic inline in the terminal
```
...
```bash
npx @hanzo/frame benchmark              # run the preset matrix in current project
npx @hanzo/frame benchmark ./my-video   # specific project
npx @hanzo/frame benchmark --runs 5     # repeat each config N times (default 3)
npx @hanzo/frame benchmark --json
```
...
```bash
npx @hanzo/frame telemetry status      # show telemetry state
npx @hanzo/frame telemetry disable     # disable anonymous usage telemetry
npx @hanzo/frame telemetry enable      # re-enable telemetry
```
...
```bash
npx @hanzo/frame tts
npx @hanzo/frame transcribe
npx @hanzo/frame remove-background
```
