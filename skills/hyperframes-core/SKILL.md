...
- [ ] `npx @hanzo/frames check` passes (0 findings across lint, runtime, layout, motion, and contrast)
- [ ] Projects with sub-compositions: `npx @hanzo/frames snapshot --at <midpoints>` and eyeball each frame
- [ ] `npx @hanzo/frames preview` for review (the user can edit anything in Studio's timeline)
- [ ] `npx @hanzo/frames render` only after the user approves
