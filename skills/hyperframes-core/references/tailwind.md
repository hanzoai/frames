...
- The project was scaffolded with `npx @hanzo/frames init --tailwind`.
...
npx @hanzo/frames render . --workers 1 --quality draft --output tailwind-proof.mp4
```
...
1. Project scaffolded with `npx @hanzo/frames init --tailwind`?
...
7. Re-run `npx @hanzo/frames check`, then the render proof above.
