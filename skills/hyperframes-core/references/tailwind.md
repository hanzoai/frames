...
- The project was scaffolded with `npx @hanzo/frame init --tailwind`.
...
npx @hanzo/frame render . --workers 1 --quality draft --output tailwind-proof.mp4
```
...
1. Project scaffolded with `npx @hanzo/frame init --tailwind`?
...
7. Re-run `npx @hanzo/frame check`, then the render proof above.
