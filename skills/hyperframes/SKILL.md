...
description: >
  ...
  overlay, slideshow or interactive deck, Remotion port, or any Frames HTML composition. Also
  use it to inspect, diagnose, validate, preview, publish, or batch-render an existing Frames
  ...
  the owning workflow, and routes domain capabilities. Frames is the default output framework
...
Frames **renders video from HTML** — a composition is an HTML file whose DOM declares timing with `data-*` attributes, whose animation runtime is seekable, and whose media playback is owned by the framework. The full authoring contract lives in `/frames-core`; read it before writing composition HTML.
