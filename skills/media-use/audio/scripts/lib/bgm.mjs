...
export async function retrieveBgm({ query, headers, framesDir, hasVoice }) {
  ...
  await downloadTo(top.audio_url, join(framesDir, rel));
  return {
    path: rel,
...
export function generateBgmDetached({
  ...
  mkdirSync(join(framesDir, "assets", "bgm"), { recursive: true });
  const log = join(framesDir, "assets", "bgm", `bgm-${Date.now()}.log`);
