...
// pipeline. The credential resolver matches the frames CLI auth: first
// usable source wins — $HEYGEN_API_KEY / $FRAMES_API_KEY → a nearby .env → ~/.heygen/
...
export function heygenCredential() {
  const envKey = process.env.HEYGEN_API_KEY || process.env.FRAMES_API_KEY;
...
export function heygenAuthHeaders() {
  ...
  if (cred?.expired)
    throw new Error(
      "HeyGen OAuth token expired — run `npx frames auth refresh` (or `npx frames auth login`)",
    );
  throw new Error(
    "no HeyGen credentials — set $HEYGEN_API_KEY, or run `npx frames auth login` (writes ~/.heygen/credentials)",
  );
}
