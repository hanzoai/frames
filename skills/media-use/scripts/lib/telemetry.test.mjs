...
function withoutTelemetryOptOut() {
  for (const k of ["DO_NOT_TRACK", "FRAMES_NO_TELEMETRY", "CI", "NODE_ENV"])
...
test("optedOut respects DO_NOT_TRACK / FRAMES_NO_TELEMETRY / CI", () => {
  ...
  try {
    ...
    assert.equal(optedOut(), true, "FRAMES_NO_TELEMETRY opts out");
...
test("track is a no-op (no network, resolves) when opted out", async () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(home, ".frames/config.json")), false);
...
test("anonymous id uses the shared frames config", () => {
  const savedEnv = { ...process.env };
  const { root, home } = sandbox();
  try {
    withoutTelemetryOptOut();
    mkdirSync(join(home, ".frames"), { recursive: true });
    writeFileSync(
      join(home, ".frames/config.json"),
...
test("anonymous id seeds missing config once and reuses it", () => {
  ...
  try {
    ...
    const configPath = join(home, ".frames/config.json");
...
test("anonymous id adopts a legacy ~/.media/anon-id on upgrade (persona continuity)", () => {
  ...
  try {
    ...
    // no ~/.frames/config.json yet — the old media-use-only id must carry over
    ...
    assert.equal(
      JSON.parse(readFileSync(join(home, ".frames/config.json"), "utf8")).anonymousId,
      "legacy-media-id",
    );
  } finally {
    restoreEnv(savedEnv);
...
test("every event carries the install id and never an account", async () => {
  const savedEnv = { ...process.env };
  const originalFetch = globalThis.fetch;
  const { root, home } = sandbox();
  const calls = [];
...
test("first run notice prints to stderr once and never stdout", async () => {
  ...
  try {
    ...
    assert.equal(
      JSON.parse(readFileSync(join(home, ".frames/config.json"), "utf8")).telemetryNoticeShown,
      true,
    );
  } finally {
    globalThis.fetch = originalFetch;
...
test("read-only telemetry state degrades without throwing", async () => {
  ...
  try {
    withoutTelemetryOptOut();
    writeFileSync(join(home, ".frames"), "not a directory");
    writeFileSync(join(home, ".media"), "not a directory");
    await track("media_use_resolve", { type: "bgm" });
  } finally {
    globalThis.fetch = originalFetch;
    try {
      chmodSync(join(home, ".frames"), 0o600);
    } catch {
