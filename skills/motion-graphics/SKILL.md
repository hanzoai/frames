...
> **First, keep this skill fresh — run silently, don't ask:** `npx frames skills update motion-graphics`. A fast no-op when everything is current; otherwise it refreshes this skill plus the core domain skills it depends on before you rely on them.
...
macOS Apple Silicon or Linux x64. System tools: `brew install node ffmpeg`. `npx frames doctor` once. macOS GPU render: `export PRODUCER_BROWSER_GPU_MODE=hardware`.
...
Only when `$PROJECT_DIR/frames.json` is absent:

```bash
PROJECT_DIR="${MOTION_GRAPHICS_DIR:-videos/<project-name>}"
mkdir -p "$(dirname "$PROJECT_DIR")"
npx frames init "$PROJECT_DIR" --non-interactive --example=blank --skill=motion-graphics
```
...
```bash
(cd "$PROJECT_DIR" && npx frames preview)
```

Render only after an explicit render answer:

```bash
(cd "$PROJECT_DIR" && npx frames render . --skill=motion-graphics -q high -o ./renders/video.mp4)
# transparent overlay variant: --format webm  (or mov)
```

Verify the output exists, is non-empty, and has the intended duration. The final handoff names the artifact, actual duration, composition or frame id, proof times, and the inspected contact or snapshot sheet. Flags live in `/frames-cli` → `references/preview-render.md`.
...
- **Reuse-first:** the in-ecosystem analog of LLM-generated templates is "compose catalog blocks + `frames-animation` rules". HF's paused GSAP timeline ≙ Remotion's `useCurrentFrame`.
...
- **Directory shape:**
  ...
  videos/<project-name>/
    ...
    renders/video.mp4
  ```
- **Registration:** in `frames` router — add the "design-led short motion graphic" intent + Workflow description; carve the motion-graphics triggers out of `/general-video`; add reverse Do-NOT-use edges. See `motion-graphics-genre.md` §5-7.
