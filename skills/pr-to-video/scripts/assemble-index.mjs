...
// Frames project. No subagent, no judgment: turns STORYBOARD.md + the
...
function ensureBgmCovers(relPath, framesDir, total) {
  const abs = join(framesDir, relPath);
  ...
  const absOut = join(framesDir, relOut);
...
const audioMetaPath = resolve(flag("audio-meta", join(framesDir, "audio_meta.json")));
const outPath = resolve(flag("out", join(framesDir, "index.html")));
...
for (const f of manifest.frames) {
  ...
  const compAbs = join(framesDir, f.src);
...
for (const m of mounted) {
  ...
  if (v?.path) {
    if (existsSync(join(framesDir, v.path))) {
      body.push(
        `      <audio`,
...
let bgmNote = "";
if (audio.bgm?.path) {
  if (existsSync(join(framesDir, audio.bgm.path))) {
    let bgmSrc = audio.bgm.path;
    const cov = ensureBgmCovers(audio.bgm.path, framesDir, TOTAL);
    if (cov.looped) {
      bgmSrc = cov.rel;
...
if (existsSync(join(framesDir, "compositions/captions.html"))) {
  body.push(
    `      <!-- captions -->`,
    `      <div`,
    `        id="el-captions"`,
...
audio.sfx.forEach((cue, i) => {
  ...
  const rel = cue.file;
  if (!existsSync(join(framesDir, rel))) {
