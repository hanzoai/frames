...
//
//   node transitions.mjs inject --storyboard ./STORYBOARD.md --frames .
...
function extendFrameTail(framesDir, frame, baseDuration, targetDuration, die) {
  if (!frame?.src || targetDuration <= baseDuration) return;
  const framePath = join(framesDir, frame.src);
  let html;
  try {
...
function runInject(argv) {
  ...
  const indexPath = join(framesDir, "index.html");
  const die = (msg) => {
  ...
  for (let i = 1; i < order.length; i++) {
    ...
    extendFrameTail(framesDir, order[i - 1].frame, baseDuration, outgoing.duration, die);
...
function runVerify(argv) {
  const framesDir = resolve(flag(argv, "frames", "."));
  const storyboardPath = resolve(flag(argv, "storyboard", join(framesDir, "STORYBOARD.md")));
  const indexPath = resolve(flag(argv, "index", join(framesDir, "index.html")));
  const bail = (msg) => {
