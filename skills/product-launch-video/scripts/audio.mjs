...
//
//   node audio.mjs --script ./SCRIPT.md --storyboard ./STORYBOARD.md --frames . --out ./audio_meta.json
//   node audio.mjs sync-durations --audio-meta ./audio_meta.json --storyboard ./STORYBOARD.md
//   node audio.mjs fetch-sfx --storyboard ./STORYBOARD.md --frames .
...
function runEngine({ request, framesDir, neutral, only, extra = [] }, die) {
  const reqPath = join(framesDir, "audio_request.json");
...
function runGenerate(argv) {
  ...
  runEngine({ request, framesDir, neutral, only: "tts,bgm" }, die);
...
function runFetchSfx(argv) {
  ...
  const framesDir = resolve(flag(argv, "frames", "."));
  const storyboardPath = resolve(flag(argv, "storyboard", join(framesDir, "STORYBOARD.md")));
  const outPath = resolve(flag(argv, "audio-meta", join(framesDir, "audio_meta.json")));
  ...
  runEngine({ request, framesDir, neutral, only: "sfx" }, die);
