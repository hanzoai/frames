...
| `clockWipe()`                      | use HF's `sdf-iris` shader-transition (`npx @hanzo/frames add sdf-iris`)                  |
...
(iris, ripple, zoom, glitch), use HF's [shader-transitions](https://frames.hanzo.ai/catalog/blocks)
package. They produce richer output than manual GSAP transforms.

```bash
npx @hanzo/frames add sdf-iris
```
